...
object BetaReduction {
    ...
    def betaNormalize(expression: LambdaExpression, strategy: StrategyOuterInner.Value):LambdaExpression = expression match {
    ...
    def betaReduce(expression: LambdaExpression, strategyOI: StrategyOuterInner.Value, strategyLR: StrategyLeftRight.Value):LambdaExpression = expression match {
        case App(Abs(x,body),arg) => {
                strategyOI match {
                    case StrategyOuterInner.Outermost => {
                            val sigma: Substitution = (x,arg)
