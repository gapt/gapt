  ...
  proof match {
    case Axiom(so : LabelledSequentOccurrence) => {
      ...
      val a = Axiom(Sequent(ant_occs.map( fo => subst(fo.formula).asInstanceOf[HOLFormula] ), succ_occs.map( fo => subst(fo.formula).asInstanceOf[HOLFormula] ) ),
        Pair( ant_occs.map( fo => fo.label.map( t => subst.apply(t) ) ),
              succ_occs.map( fo => fo.label.map( t => subst.apply(t) ) ) ) )
    ...
    case WeakeningLeftRule(p, s, m) => {
      ...
      val new_proof = WeakeningLeftRule( new_parent._1, subst(m.formula).asInstanceOf[HOLFormula], m.label.map( e => subst.apply(e) ) )
    ...
    case WeakeningRightRule(p, s, m) => {
      val new_parent = apply( p, subst )
      val new_proof = WeakeningRightRule( new_parent._1, subst(m.formula).asInstanceOf[HOLFormula], m.label.map( e => subst.apply(e) ) )
      val es = toLabelledSequentOccurrence( p.root )
      ( new_proof, computeMap( es.l_antecedent ++ es.l_succedent, proof, new_proof, new_parent._2 ) + Pair(m, new_proof.prin.first ) )
    }
    case ForallSkLeftRule(p, s, a, m, t) => {
      val new_parent = apply( p, subst )
      val new_proof = ForallSkLeftRule( new_parent._1, new_parent._2(a), subst(m.formula).asInstanceOf[HOLFormula], subst.apply(t), a.label.contains(t) )
      val es = toLabelledSequentOccurrence( p.root )
      ( new_proof, computeMap( es.l_antecedent ++ es.l_succedent, proof, new_proof, new_parent._2 ) )
    }
    case ExistsSkRightRule(p, s, a, m, t) => {
      val new_parent = apply( p, subst )
      val new_proof = ExistsSkRightRule( new_parent._1, new_parent._2(a), subst(m.formula).asInstanceOf[HOLFormula], subst.apply(t), a.label.contains(t) )
      val es = toLabelledSequentOccurrence( p.root )
      ( new_proof, computeMap( es.l_antecedent ++ es.l_succedent, proof, new_proof, new_parent._2 ) )
    }
