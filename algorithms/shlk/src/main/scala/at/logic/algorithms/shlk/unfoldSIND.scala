...
object applySchemaSubstitution2 {
  ...
  def handleWeakening( new_parent: LKProof,
                       ...
                       old_proof: LKProof,
                       constructor: (LKProof, HOLFormula) => LKProof with PrincipalFormulas,
                       m: FormulaOccurrence, trs: dbTRS ) = {
    val new_proof = constructor( new_parent, unfoldSFormula(subst(m.formula).asInstanceOf[HOLFormula], trs) )
    new_proof
  }
  def handleContraction( new_parent: LKProof,
                         subst: SchemaSubstitution1[HOLExpression],
    ...
    //    println("aux = "+subst(a1.formula))
    //    println("seq = "+new_parent.root)
    constructor( new_parent, unfoldSFormula(subst(a1.formula).asInstanceOf[HOLFormula], trs), trs )
    //    ( new_proof, computeMap( old_parent.root.antecedent ++ old_parent.root.succedent, old_proof, new_proof, new_parent._2 ) )
  }
  def handleBinaryProp( new_parent_1: LKProof,
                        new_parent_2: LKProof,
                        ...
                        old_parent_2: LKProof,
                        old_proof: LKProof,
                        constructor: (LKProof, LKProof, HOLFormula, HOLFormula) => LKProof, trs: dbTRS) = {

    constructor( new_parent_1, new_parent_2, unfoldSFormula(subst(a1.formula).asInstanceOf[HOLFormula], trs), unfoldSFormula(subst(a2.formula).asInstanceOf[HOLFormula], trs) )
  }
  ...
  def handleRule( proof: LKProof, new_parents: List[LKProof], subst: SchemaSubstitution1[HOLExpression], trs: dbTRS ) : LKProof = {
    ...
    proof match {
      ...
      case Axiom(ro) => {
        val a = Axiom(ro.antecedent.map(fo => unfoldSFormula(subst(fo.formula).asInstanceOf[HOLFormula], trs)),ro.succedent.toList.map(fo => unfoldSFormula(subst(fo.formula).asInstanceOf[HOLFormula], trs)))
        //        val ant_occs = a._1.root.antecedent.toList
        //        val succ_occs = a._1.root.succedent.toList
        val map = new HashMap[FormulaOccurrence, FormulaOccurrence]
        //        a._2._1.zip(a._2._1.indices).foreach( p => map.update( ant_occs( p._2 ), p._1 ) )
      ...
      case WeakeningLeftRule(p, s, m) => handleWeakening( new_parents.head, subst, p, proof, WeakeningLeftRule.apply, m, trs )
      case WeakeningRightRule(p, s, m) => handleWeakening( new_parents.head, subst, p, proof, WeakeningRightRule.apply, m, trs )
      case ContractionLeftRule(p, s, a1, a2, m) => handleContraction( new_parents.head, subst, p, proof, a1, a2, sContractionLeftRule.apply, trs )
      case ContractionRightRule(p, s, a1, a2, m) => handleContraction( new_parents.head, subst, p, proof, a1, a2, sContractionRightRule.apply, trs)
      case CutRule(p1, p2, s, a1, a2) => {
        val new_p1 = new_parents.head
        ...
        //        println(new_p2.root)
        //val new_proof = CutRule( new_p1._1, new_p2._1, new_p1._2( a1 ), new_p2._2( a2 ) )
        val new_proof = sCutRule( new_p1, new_p2, unfoldSFormula(subst(a1.formula).asInstanceOf[HOLFormula], trs), trs )
        //    ( new_proof, computeMap(
        //     p1.root.antecedent ++ (p1.root.succedent - a1) ++ (p2.root.antecedent - a2) ++ p2.root.succedent,
        //    proof, new_proof, new_p1._2 ++ new_p2._2 ) )
        new_proof
      }
      case AndRightRule(p1, p2, s, a1, a2, m) => handleBinaryProp( new_parents.head, new_parents.last, subst, a1, a2, p1, p2, proof, AndRightRule.apply, trs )

      case AndLeft1Rule(p, s, a, m) => {
        val f = m.formula match { case And(_, w) => w }
        val new_parent = new_parents.head
        val new_proof = AndLeft1Rule( new_parent, unfoldSFormula(subst(a.formula).asInstanceOf[HOLFormula], trs), unfoldSFormula(subst(f.asInstanceOf[HOLFormula]).asInstanceOf[HOLFormula], trs) )
        new_proof
      }
      case AndLeft2Rule(p, s, a, m) => {
        val f = m.formula match { case And(w, _) => w }
        val new_parent = new_parents.head
        //        val new_proof = AndLeft2Rule( new_parent._1, subst( f ).asInstanceOf[HOLFormula], new_parent._2( a ) )
        val new_proof = AndLeft2Rule( new_parent, unfoldSFormula(subst(f.asInstanceOf[HOLFormula]).asInstanceOf[HOLFormula], trs), unfoldSFormula(subst(a.formula).asInstanceOf[HOLFormula], trs) )
        new_proof
      }
      //    case OrLeftRule(p1, p2, s, a1, a2, m) => handleBinaryProp( new_parents.head, new_parents.last, subst, a1, a2, p1, p2, proof, OrLeftRule.apply )
      ...
      case OrRight1Rule(p, s, a, m) => {
        ...
        val new_parent = new_parents.head
        //        val new_proof = OrRight1Rule( new_parent._1, new_parent._2( a ), subst( f ).asInstanceOf[HOLFormula] )
        val new_proof = OrRight1Rule( new_parent, unfoldSFormula(subst(a.formula).asInstanceOf[HOLFormula], trs), unfoldSFormula(subst(f.asInstanceOf[HOLFormula]).asInstanceOf[HOLFormula], trs) )
        new_proof
      }
      case OrRight2Rule(p, s, a, m) => {
        val f = m.formula match {
        ...
        val new_parent = new_parents.head
        val new_proof = OrRight2Rule( new_parent, unfoldSFormula(subst(f.asInstanceOf[HOLFormula]).asInstanceOf[HOLFormula], trs), unfoldSFormula(subst(a.formula).asInstanceOf[HOLFormula], trs) )
        new_proof
      }
      case NegLeftRule(p, s, a, m) => {
        val new_parent = new_parents.head
        //  val new_proof = NegLeftRule( new_parent._1, new_parent._2( a ) )
        val new_proof = NegLeftRule( new_parent, unfoldSFormula(subst(a.formula).asInstanceOf[HOLFormula], trs) )
        //       ( new_proof, computeMap( p.root.antecedent ++ p.root.succedent, proof, new_proof, new_parent._2 ) )
        new_proof
      }
      case NegRightRule(p, s, a, m) => {
        ...
        val map = new HashMap[FormulaOccurrence, FormulaOccurrence]
        //val new_proof = NegRightRule( new_parent._1, new_parent._2( a ) )
        val new_proof = NegRightRule( new_parent, unfoldSFormula(subst(a.formula).asInstanceOf[HOLFormula], trs) )
        //     ( new_proof, computeMap( p.root.antecedent ++ p.root.succedent, proof, new_proof, new_parent._2 ) )
        new_proof
      }
      case ImpLeftRule(p1, p2, s, a1, a2, m) => {
        val new_parent1 = new_parents.head
        val new_parent2 = new_parents.last
        ImpLeftRule(new_parent1, new_parent2, unfoldSFormula(subst(a1.formula).asInstanceOf[HOLFormula], trs), unfoldSFormula(subst(a2.formula).asInstanceOf[HOLFormula], trs))
      }
      case ImpRightRule(p, s, a1, a2, m) => {
        val new_parent = new_parents.head
        ImpRightRule(new_parent, unfoldSFormula(subst(a1.formula).asInstanceOf[HOLFormula], trs), unfoldSFormula(subst(a2.formula).asInstanceOf[HOLFormula], trs))
      }
      case ForallLeftRule(p, seq, a, m, t) => {
        val new_parent = new_parents.head
...
//        println(printSchemaProof.formulaToString(unfoldSTerm(subst(t), trs))+Console.RESET)
        ...
        ForallLeftRule(new_parent, unfoldSFormula(subst(a.formula).asInstanceOf[HOLFormula], trs), unfoldSFormula(subst(m.formula).asInstanceOf[HOLFormula], trs), unfoldSTerm(subst(t), trs))
      }
      case ForallRightRule(p, seq, a, m, v) => {
        val new_parent = new_parents.head
...
//        println("v : "+printSchemaProof.formulaToString(subst(v).asInstanceOf[HOLVar]))
        ForallRightRule(new_parent, unfoldSFormula(subst(a.formula).asInstanceOf[HOLFormula], trs), unfoldSFormula(subst(m.formula).asInstanceOf[HOLFormula], trs), subst(v).asInstanceOf[HOLVar])
      }
    }
  }
...
//        trsArrowRule(apply(up, subst, cnt, trs), unfoldSFormula(subst(aux.formula).asInstanceOf[HOLFormula], trs), trs)
      ...
      case OrLeftRule(p1, p2, s, a1, a2, m) => {
        ...
        //   println("\nright proof seq:"+printSchemaProof.sequentToString(pr2._1.root))
        //  println("\naux f :"+printSchemaProof.formulaToString(subst(a1.formula))+"     |     "+printSchemaProof.formulaToString(subst(a2.formula)))
        val res = handleBinaryProp( pr1, pr2, subst, a1, a2, p1, p2, proof, OrLeftRule.apply, trs )
        //        println("\nafter : "+res.rule)
        res
      }
...
//        trsArrowRightRule(new_p, a.formula, trs)
      ...
      case Axiom(ro) => Axiom(ro.antecedent.map(fo => unfoldSFormula(fo.formula.asInstanceOf[HOLFormula], trs)),ro.succedent.map(fo => unfoldSFormula(fo.formula.asInstanceOf[HOLFormula], trs)))

      case AndLeftEquivalenceRule1(p, s, a, m) => {
        //            println("\nAndLeftEquivalenceRule1   YESSSSSSSSSSS \n")
        val new_p = apply(p, trs)
        AndLeftEquivalenceRule1(new_p, unfoldSFormula(a.formula.asInstanceOf[SchemaFormula], trs), unfoldSFormula(m.formula.asInstanceOf[SchemaFormula], trs))
      }

      case AndRightEquivalenceRule1(p, s, a, m) => {
        // println("\nAndRightEquivalenceRule1\n")
        val new_p = apply(p, trs)
        AndRightEquivalenceRule1(new_p, unfoldSFormula(a.formula.asInstanceOf[SchemaFormula], trs), unfoldSFormula(m.formula.asInstanceOf[SchemaFormula], trs))
      }

      case OrRightEquivalenceRule1(p, s, a, m) => {
        // println("\nOrRightEquivalenceRule1\n")
        val new_p = apply(p, trs)
        OrRightEquivalenceRule1(new_p, unfoldSFormula(a.formula.asInstanceOf[SchemaFormula], trs), unfoldSFormula(m.formula.asInstanceOf[SchemaFormula], trs))
      }

      case AndLeftEquivalenceRule3(p, s, a, m) => {
        // println("\nAndLeftEquivalenceRule3\n")
        val new_p = apply(p, trs)
        AndLeftEquivalenceRule3(new_p, unfoldSFormula(a.formula.asInstanceOf[SchemaFormula], trs), unfoldSFormula(m.formula.asInstanceOf[SchemaFormula], trs))
      }

      case AndRightEquivalenceRule3(p, s, a, m) => {
        // println("\nAndRightEquivalenceRule3\n")
        val new_p = apply(p, trs)
        AndRightEquivalenceRule3(new_p, unfoldSFormula(a.formula.asInstanceOf[SchemaFormula], trs), unfoldSFormula(m.formula.asInstanceOf[SchemaFormula], trs))
      }

      case OrRightEquivalenceRule3(p, s, a, m) => {
        //println("\nOrRightEquivalenceRule3\n")
        val new_p = apply(p, trs)
        OrRightEquivalenceRule3(new_p, unfoldSFormula(a.formula.asInstanceOf[SchemaFormula], trs), unfoldSFormula(m.formula.asInstanceOf[SchemaFormula], trs))
      }

      case WeakeningLeftRule(p, _, m) => {
        val new_p = apply(p, trs)
        implicit val factory = defaultFormulaOccurrenceFactory
        WeakeningLeftRule( new_p, unfoldSFormula(m.formula, trs) )
      }

      case WeakeningRightRule(p, _, m) => {
        val new_p = apply(p, trs)
        implicit val factory = defaultFormulaOccurrenceFactory
        WeakeningRightRule( new_p, unfoldSFormula(m.formula, trs) )
      }

      case CutRule( p1, p2, _, a1, a2 ) => {
        val new_p1 = apply(p1, trs)
        val new_p2 = apply(p2, trs)
        CutRule(new_p1, new_p2, unfoldSFormula(a2.formula, trs))
      }

      case OrLeftRule(p1, p2, _, a1, a2, m) => {
        val new_p1 = apply(p1, trs)
        val new_p2 = apply(p2, trs)
        OrLeftRule(new_p1, new_p2, unfoldSFormula(a1.formula, trs), unfoldSFormula(a2.formula, trs))
      }

      case AndRightRule(p1, p2, _, a1, a2, m) => {
        val new_p1 = apply(p1, trs)
        val new_p2 = apply(p2, trs)
        AndRightRule(new_p1, new_p2, unfoldSFormula(a1.formula, trs), unfoldSFormula(a2.formula, trs))
      }

      case NegLeftRule( p, _, a, m ) => {
        val new_p = apply(p, trs)
        NegLeftRule( new_p, unfoldSFormula(a.formula, trs) )
      }
      ...
      case AndLeft1Rule(p, r, a, m) =>  {
        ...
        //     println("aux : \n"+printSchemaProof.formulaToString(a.formula))
        //    println(printSchemaProof.formulaToString(a2))
        AndLeft1Rule( new_p, unfoldSFormula(a.formula, trs), unfoldSFormula(a2, trs))
      }
      ...
      case AndLeft2Rule(p, r, a, m) =>  {
        ...
        //     println("aux : \n"+printSchemaProof.formulaToString(a.formula))
        //     println(printSchemaProof.formulaToString(a2))
        AndLeft2Rule( new_p, unfoldSFormula(a2, trs), unfoldSFormula(a.formula, trs) )
      }
      ...
      case OrRight1Rule(p, r, a, m) =>  {
        ...
        //            println("\nor:r1 a = "+a.formula)
        //            println("\nor:r1 m = "+m.formula)
        OrRight1Rule( new_p, unfoldSFormula(a.formula, trs), unfoldSFormula(a2, trs))
      }
      ...
      case OrRight2Rule(p, r, a, m) =>  {
        ...
        //          println("\nor:r2 a = "+a.formula)
        //            println("\nor:r2 m = "+m.formula)
        OrRight2Rule( new_p, unfoldSFormula(a2, trs), unfoldSFormula(a.formula, trs))
      }

      case NegRightRule( p, _, a, m ) => {
        val new_p = apply(p, trs)
        NegRightRule( new_p, unfoldSFormula(a.formula, trs) )
      }

      case ContractionLeftRule(p, _, a1, a2, m) => {
        val new_p = apply(p, trs)
        ContractionLeftRule( new_p, unfoldSFormula(a1.formula, trs) )
      }

      case ContractionRightRule(p, _, a1, a2, m) => {
        val new_p = apply(p, trs)
        //            println("\nc:r = "+new_p.root)
        ContractionRightRule( new_p, unfoldSFormula(a1.formula, trs) )
      }

      case ForallLeftRule(p, seq, a, m, t) => {
        val new_parent = apply(p, trs)
        ForallLeftRule(new_parent, unfoldSFormula(a.formula, trs), unfoldSFormula(m.formula, trs), unfoldSTerm(t, trs))
      }
      case ForallRightRule(p, seq, a, m, v) => {
        val new_parent = apply(p, trs)
        ForallRightRule(new_parent, unfoldSFormula(a.formula, trs), unfoldSFormula(m.formula, trs), v)
      }

      case ImpLeftRule(p1, p2, s, a1, a2, m) => {
        val new_parent1 = apply(p1, trs)
        val new_parent2 = apply(p2, trs)
        ImpLeftRule(new_parent1, new_parent2, unfoldSFormula(a1.formula, trs), unfoldSFormula(a2.formula, trs))
      }
      case ImpRightRule(p, s, a1, a2, m) => {
        val new_parent = apply(p, trs)
        ImpRightRule(new_parent, unfoldSFormula(a1.formula, trs), unfoldSFormula(a2.formula, trs))
      }
      case _ => { println("ERROR in CloneLKProof2 : missing rule!");throw new Exception("ERROR in unfolding: CloneLKProof2: missing rule !\n") }
    }}
}
...
object LKrwToLK {
  def apply(p: LKProof, trs: dbTRS): LKProof = {
    ...
    p match {
      case Axiom(seq) => Axiom(seq.antecedent.map(f => unfoldSFormula(f.formula, trs)), seq.succedent.map(f => unfoldSFormula(f.formula, trs)))
      case trsArrowLeftRule(p, s, a, m) => {
        //            println("\nAndLeftEquivalenceRule1   YESSSSSSSSSSS \n")
        apply(p, trs)
      }
      ...
      case AndLeft1Rule(p, r, a, m) =>  {
        ...
        //     println("aux : \n"+printSchemaProof.formulaToString(a.formula))
        //    println(printSchemaProof.formulaToString(a2))
        AndLeft1Rule( new_p, unfoldSFormula(a.formula, trs), unfoldSFormula(a2.asInstanceOf[HOLFormula], trs))
      }
      ...
      case AndLeft2Rule(p, r, a, m) =>  {
        ...
        //     println("aux : \n"+printSchemaProof.formulaToString(a.formula))
        //     println(printSchemaProof.formulaToString(a2))
        AndLeft2Rule( new_p, unfoldSFormula(a2.asInstanceOf[HOLFormula], trs), unfoldSFormula(a.formula, trs) )
      }
      ...
      case OrRight1Rule(p, r, a, m) =>  {
        ...
        //            println("\nor:r1 a = "+a.formula)
        //            println("\nor:r1 m = "+m.formula)
        OrRight1Rule( new_p, unfoldSFormula(a.formula, trs), unfoldSFormula(a2.asInstanceOf[HOLFormula], trs))
      }
      ...
      case OrRight2Rule(p, r, a, m) =>  {
        ...
        //          println("\nor:r2 a = "+a.formula)
        //            println("\nor:r2 m = "+m.formula)
        OrRight2Rule( new_p, unfoldSFormula(a2.asInstanceOf[HOLFormula], trs), unfoldSFormula(a.formula, trs))
      }
