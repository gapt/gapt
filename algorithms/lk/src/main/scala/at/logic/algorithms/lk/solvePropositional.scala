...
object solvePropositional {
  ...
  def prove(seq: FSequent): Option[LKProof] = {
    ...
    // TODO: solve in this order:
    // Check if its axiom
    // Choose unary rules (left or right, don't care)
    // Choose binary rule (left or right, don't care)

    // Solve something on the left
    else if (getNonAtomicFAnt(seq) != None) {
      val f = getNonAtomicFAnt(seq).get._1
      val rest = getNonAtomicFAnt(seq).get._2
      // Solving a left formula
      f match {
        ...
        case _ => throw new Exception("ERROR: Unexpected connective while solving propositional sequent.")
      }
    }

    // Solve something on the right
    else if (getNonAtomicFSucc(seq) != None) {
  ...
  def findUnary(seq: FSequent) : Option[HOLFormula] = {
    val left = seq.antecedent.find(f => f match {
  ...
  def findBinary(seq: FSequent) : Option[HOLFormula] = {
    val left = seq.antecedent.find(f => f match {
  ...
  def getNonAtomicFSucc(seq: FSequent) : Option[(HOLFormula, FSequent)] = {
    seq.succedent.foreach(f => f match {
      ...
      case Atom(_ , arg) => {
        if ( arg.head.exptype == Ti() )
          {  }
        else return Some(f, removeFfromSeqSucc(seq, f))
      }
