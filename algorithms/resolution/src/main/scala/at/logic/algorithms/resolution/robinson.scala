...
import at.logic.algorithms.lk.{WeakeningsElimination, applySubstitution}

object RobinsonToLK {
  // if the proof can be obtained from the CNF(-s) then we compute an LKProof of |- s
  // Important! we use WeakeningsElimination to remove from the end sequent all formulas generated by weakening
  // this is done because of the multipicative rules but we might need to add weakenings at the end!
  def apply(resproof: RobinsonResolutionProof, s: FSequent): LKProof = WeakeningsElimination(recConvert(resproof, Substitution[FOLExpression](), s))
