...
package logical {
import _root_.at.logic.calculi.lk.base.SequentOccurrence
...
import at.logic.provers.atp.Definitions._
  ...
  case class DeterministicAndCommand[V <: SequentOccurrence](commands: Tuple2[Iterable[DataCommand[V]],Iterable[DataCommand[V]]]) extends DataCommand[V] {
  ...
  // using the same state, therefore deterministic
  abstract class DeterministicMacroCommand [V <: SequentOccurrence](init: InitialCommand[V], datas: Iterable[DataCommand[V]], result: ResultCommand[V]) extends Command[V] with
