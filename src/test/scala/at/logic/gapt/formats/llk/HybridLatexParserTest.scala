...
class HybridLatexParserTest extends Specification with ClasspathFileCopier {
  ...
  "Tactics" should {
    "correctly prove the instance of an axiom" in {
      ...
      val axiom = LLKFormulaParser.ASTtoHOL( naming, HybridLatexParser.parseFormula( "(all x all y all z (x+(y+z)=(x+y)+z))" ) )
      val instance = LLKFormulaParser.ASTtoHOL( naming, HybridLatexParser.parseFormula( "a+((1+x)+y)=(a+(1+x))+y" ) )
