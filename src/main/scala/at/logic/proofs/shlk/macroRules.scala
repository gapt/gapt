...
object AndEquivalenceRule {
  def apply( p: LKProof, auxf: FormulaOccurrence, main: SchemaFormula ) = {
    main match {
      case BigAnd( v, f, ub, Succ( lb ) ) if And( BigAnd( v, f, ub, lb ), betaNormalize( SchemaApp( SchemaAbs( v, f ), Succ( lb ) ) ).asInstanceOf[SchemaFormula] ) == auxf.formula =>
        AndEquivalenceRule1( p, auxf, main )
      case BigAnd( v, f, ub, lb ) if And( BigAnd( v, f, Succ( ub ), lb ), betaNormalize( SchemaApp( SchemaAbs( v, f ), ub ) ).asInstanceOf[SchemaFormula] ) == auxf.formula =>
