...
import at.logic.gapt.language.lambda.types.{ To, FunctionType, Tindex, Ti }
...
object SHLK {
  ...
  def parseSequent( txt: String ): FSequent = {
    ...
    class SequentParser extends JavaTokenParsers with at.logic.gapt.language.lambda.types.Parsers {
      ...
      def imp: Parser[SchemaFormula] = "Imp" ~ formula ~ formula ^^ { case "Imp" ~ x ~ y => SchemaImp( x, y ) }
  ...
  def parseProof( txt: InputStreamReader ): MMap[String, Tuple2[MMap[String, LKProof], MMap[String, LKProof]]] = {
    ...
    //    class ParserTxt extends JavaTokenParsers with at.logic.gapt.language.lambda.types.Parsers {
    //
