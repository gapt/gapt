package at.logic.gapt.language.hol.unification
...
import at.logic.gapt.language.hol.unification

import scala.collection.mutable.{ HashSet => MHashSet }

trait DiophantineSolver[Data] {
  def solve( coeff_lhs: at.logic.gapt.language.hol.unification.Vector, coeff_rhs: at.logic.gapt.language.hol.unification.Vector ): List[at.logic.gapt.language.hol.unification.Vector]
}
...
case class LankfordSolverInstance( val a: at.logic.gapt.language.hol.unification.Vector, val b: at.logic.gapt.language.hol.unification.Vector ) {
  ...
  def norm( v: at.logic.gapt.language.hol.unification.Vector ) = ab * v
  ...
  def solve = {
    ...
    val a = new MHashSet[at.logic.gapt.language.hol.unification.Vector]
    val b = new MHashSet[at.logic.gapt.language.hol.unification.Vector]

    for ( i <- sim._1 )
      a += new at.logic.gapt.language.hol.unification.Vector( i )
    for ( i <- sim._2 )
      b += new at.logic.gapt.language.hol.unification.Vector( i )

    val positive: MHashSet[at.logic.gapt.language.hol.unification.Vector] = new MHashSet[at.logic.gapt.language.hol.unification.Vector]
    val negative: MHashSet[at.logic.gapt.language.hol.unification.Vector] = new MHashSet[at.logic.gapt.language.hol.unification.Vector]
    val zero: MHashSet[at.logic.gapt.language.hol.unification.Vector] = new MHashSet[at.logic.gapt.language.hol.unification.Vector]
    var x: MHashSet[at.logic.gapt.language.hol.unification.Vector] = new MHashSet[at.logic.gapt.language.hol.unification.Vector]
    var zero_new: MHashSet[at.logic.gapt.language.hol.unification.Vector] = null
    var stage = 1
    ...
    while ( !positive.isEmpty || !negative.isEmpty ) {
      ...
      zero_new = new MHashSet[at.logic.gapt.language.hol.unification.Vector]
      for ( s <- x ) {
        val ns = norm( s )
  ...
  def reduceVector( v: at.logic.gapt.language.hol.unification.Vector, zero: List[at.logic.gapt.language.hol.unification.Vector] ) = {
    var w = v
    var temp = v.zero
    for ( i <- zero ) {
      temp = w - i
  ...
  def z_irreducible( s: at.logic.gapt.language.hol.unification.Vector, zero: Iterable[at.logic.gapt.language.hol.unification.Vector] ): Boolean = {
    var r = true
    for ( z <- zero )
      if ( s >= z )
        r = false
