...
object toPrettyString {
  ...
  def string_of_expr( e: LambdaExpression, use_parens: Boolean = false ): String = e match {
    ...
    case Ex( x, f )                => opt_parens( ExistsQ.name + string_of_expr( x, false ) + "." + string_of_expr( f, true ), use_parens )
    case All( x, f )               => opt_parens( ForallQ.name + string_of_expr( x, false ) + "." + string_of_expr( f, true ), use_parens )
