...
object SchemaFactory extends FactoryA {
  ...
  def createConnective( sym: SymbolA, tp: TA = Ti ): SchemaConst = sym match {
    case BottomSymbol => SchemaBottomC
    case TopSymbol    => SchemaTopC
    case NegSymbol    => SchemaNegC
    case AndSymbol    => SchemaAndC
    case OrSymbol     => SchemaOrC
    case ImpSymbol    => SchemaImpC
    case EqSymbol     => SchemaEqC( tp )
    case ForallSymbol => SchemaAllQ( tp )
    case ExistsSymbol => SchemaExQ( tp )
