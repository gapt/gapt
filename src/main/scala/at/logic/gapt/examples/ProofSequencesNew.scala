...
import at.logic.gapt.proofs.lkNew.{ LKProof => newLKProof, _ }

trait ProofSequenceNew {
  def apply( n: Int ): newLKProof
...
object LinearExampleProofNew extends ProofSequenceNew {
  val s = "s"
  val p = "P"
  val c = "0"
  ...
  def proof( k: Int, n: Int ): newLKProof =
    {
      val x = FOLVar( "x" )
...
object SquareDiagonalExampleProofNew extends ProofSequenceNew {
  val s = "s"
  val p = "P"
  val c = "0"
...
object SquareEdgesExampleProofNew extends ProofSequenceNew {
  val s = "s"
  val p = "P"
  val c = "0"
  ...
  def upper_proof( k: Int, n: Int ): newLKProof =
    {
      if ( k == n ) // leaf proof
      {
        val a = FOLAtom( p, Utils.numeral( n ) :: Utils.numeral( n ) :: Nil )
...
object SquareEdges2DimExampleProofNew extends ProofSequenceNew {
...
//
// where n is an Integer parameter >= 0.
object SumExampleProofNew extends ProofSequenceNew {
  val s = "s"
  val p = "P"
...
//
// where n is an Integer parameter >= 0.
object LinearEqExampleProofNew extends ProofSequenceNew {
  val a = "a"
  val f = "f"
...
object SumOfOnesF2ExampleProofNew extends ProofSequenceNew {
  val s = "s"
  val zero = "0"
  val p = "+"
  var f = "f"
   ...
   */
  def RecProof( s1: newLKProof, n: Int ): newLKProof = {
    if ( n <= 0 ) { s1 }
    else {
 ...
 */
object SumOfOnesFExampleProofNew extends ProofSequenceNew {
  val s = "s"
  val zero = "0"
  val p = "+"
  var f = "f"
   ...
   */
  def EqChainProof( s1: newLKProof, n: Int ): newLKProof = {
    if ( n <= 0 ) { s1 }
    else {
   ...
   * TODO should be private - but scala shell does not allow access modifiers when :loading a file
   */
  def TermGenProof( s1: newLKProof, n: Int, targetN: Int ): newLKProof = {
    if ( n >= targetN ) { s1 }
    else {
...
//
// where n is an Integer parameter >= 0.
object SumOfOnesExampleProofNew extends ProofSequenceNew {
  val s = "s"
  val zero = "0"
  val p = "+"
  ...
  // TODO should be private - but scala shell does not allow access modifiers when :loading a file
  def aux_proof( k: Int ): newLKProof = {
    /// atoms
 ...
 */
class AllQuantifiedConditionalAxiomHelperNew( variables: List[FOLVar], conditions: List[FOLAtom], consequence: FOLFormula ) {
  /**
   ...
   */
  def apply( expressions: List[FOLTerm], p: newLKProof ): newLKProof = {
    ...
    def instantiate_axiom( expressions: List[FOLTerm], axiom: FOLFormula, p: newLKProof ): newLKProof = {
      expressions match {
        case Nil => p
        case head :: tail =>
  ...
  private def apply_conditional_equality( equalities: List[FOLAtom], result: FOLFormula, p: newLKProof ): newLKProof = {
    equalities match {
      case Nil =>
...
object UniformAssociativity3ExampleProofNew extends ProofSequenceNew {
  ...
  // TODO: port these axioms to new format using AllQuantifiedConditionalAxiomHelperNew
  ...
  val cp = new AllQuantifiedConditionalAxiomHelperNew( x1 :: x2 :: y1 :: y2 :: Nil, Eq( x1, x2 ) :: Eq( y1, y2 ) :: Nil, Eq( FOLFunction( p, x1 :: y1 :: Nil ), FOLFunction( p, x2 :: y2 :: Nil ) ) )
  ...
  def addAllAxioms( proof: newLKProof ): newLKProof =
   ...
   */
  def induction_start( n: Int, p0: newLKProof ): newLKProof = {
   ...
   * (n + n) + i   = n + (n + i), Ax |- ...
   */
  def gen_proof_step( i: Int, n: Int ): newLKProof = {
    val n_num = Utils.numeral( n )
  ...
  def show_by_ax2( x: FOLTerm, y: FOLTerm, p: newLKProof ): newLKProof = {
    val p1 = ForallLeftRule( p, ax2_ax( x ), y )
    val p2 = ForallLeftRule( p1, ax2_ax(), x )
    ContractionLeftRule( p2, ax2_ax() )
  }

  def show_by_cs( x: FOLTerm, y: FOLTerm, p: newLKProof ): newLKProof = {
   ...
   * (y=x), ... |- ...
   */
  def reflect( x: FOLTerm, y: FOLTerm, p: newLKProof ): newLKProof = {
  ...
  def apply_conditional_equality( equalities: List[FOLFormula], result: FOLFormula, p: newLKProof ): newLKProof = {
    equalities match {
      case Nil => p
      case head :: Nil => {
 ...
 */
object FactorialFunctionEqualityExampleProofNew extends ProofSequenceNew {
  ...
  val g_ax_1 = new AllQuantifiedConditionalAxiomHelperNew( y :: Nil, Nil, Eq( y, f2( g, Utils.numeral( 0 ), y ) ) )
  val g_ax_2 = SimpleFOLParser( "Forall x Forall y =( g(s(x), y), g(x, *(y , s(x))) )" )

  val g_compat_2 = new AllQuantifiedConditionalAxiomHelperNew( x :: y :: z :: Nil, Eq( y, z ) :: Nil, Eq( f2( g, x, y ), f2( g, x, z ) ) )

  val trans_axiom = new AllQuantifiedConditionalAxiomHelperNew( x :: y :: z :: Nil, Eq( x, y ) :: Eq( y, z ) :: Nil, Eq( x, z ) )
  val symm_axiom = All( x, Eq( x, x ) )
  val refl_axiom = new AllQuantifiedConditionalAxiomHelperNew( x :: y :: Nil, Eq( x, y ) :: Nil, Eq( y, x ) )
  val compat_mul_axiom = new AllQuantifiedConditionalAxiomHelperNew( x :: y :: z :: Nil, Eq( x, y ) :: Nil, Eq( f2( z, m, x ), f2( z, m, y ) ) )
  val assoc_mul_axiom = new AllQuantifiedConditionalAxiomHelperNew( x :: y :: z :: Nil, Nil, Eq( f2( x, m, f2( y, m, z ) ), f2( f2( x, m, y ), m, z ) ) )

  val mul_neutral_axiom = new AllQuantifiedConditionalAxiomHelperNew( x :: Nil, Nil, Eq( f2( x, m, Utils.numeral( 1 ) ), x ) )
  // this second axiom saves us from adding commutativity of multiplication
  val mul_neutral_axiom_2 = new AllQuantifiedConditionalAxiomHelperNew( x :: Nil, Nil, Eq( f2( Utils.numeral( 1 ), m, x ), x ) )

  def apply( n: Int ): newLKProof = n match {
    case 0 =>
  ...
  def induction_steps( n: Int ): newLKProof = {
    val axiom_formulae = Eq( f1( f, Utils.numeral( n ) ), f2( g, Utils.numeral( n ), Utils.numeral( 1 ) ) ) :: Nil
    val axiom: newLKProof = Axiom( axiom_formulae, axiom_formulae )
    ...
    def induction_step_rec( p0: newLKProof, k: Int ): newLKProof = {
...
object FactorialFunctionEqualityExampleProof2New extends ProofSequenceNew {
  ...
  def apply( n: Int ): newLKProof = {
    ...
    val p6 = ( 0 until n ).foldLeft[newLKProof]( p5 ) { ( acc: newLKProof, i: Int ) =>
...
*/
    ...
    val p2 = ( 0 until n ).foldLeft[newLKProof]( p1 ) { ( acc: newLKProof, i: Int ) =>
      ( ProofBuilder
        c acc
