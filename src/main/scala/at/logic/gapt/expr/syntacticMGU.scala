  ...
  private def unify( eqs: List[( LambdaExpression, LambdaExpression )], env: Map[Var, LambdaExpression], bound: Set[Var] ): Option[Substitution] = eqs match {
    ...
    case first :: rest => first match {
      ...
      case ( Abs( v1, t1 ), Abs( v2, t2 ) ) if v1.exptype == v2.exptype =>
        val v_ = rename( v1, env.keySet ++ eqs.flatMap { p => freeVariables( p._1 ) ++ freeVariables( p._2 ) } ++ bound toList )
        unify( ( Substitution( v1 -> v_ )( t1 ) -> Substitution( v2 -> v_ )( t2 ) ) :: rest, env, bound + v_ )
      case ( x: Var, y: Var ) if x == y => unify( rest, env, bound )
      case ( x: Var, t ) if !( bound contains x ) =>
        if ( env contains x ) {
          ...
          } else if ( freeVariables( t_ ) contains x ) {
            None
          } else {
      ...
      case ( exp, v: Var ) if !( bound contains v ) => unify( ( v -> exp ) :: rest, env, bound )
      case _                                        => None
    }
  }
