 ...
 * @param atom
 */
case class LogicalAxiomTactic( atom: HOLAtom ) extends Tactic {
  ...
  override def apply( goal: OpenAssumption ) = {
    ...
    val indices =
      for {
        ( `atom`, indexAnt ) <- goalSequent.zipWithIndex.succedent
        ( `atom`, indexSuc ) <- goalSequent.zipWithIndex.antecedent
      } yield ( indexAnt, indexSuc )

    for ( _ <- indices.headOption ) yield {

      val ax = LogicalAxiom( atom )
  ...
  override def apply( goal: OpenAssumption ) = {
    if ( !( freeVariables( goal.conclusion ) contains eigenVariable ) ) {
      ...
      for ( i <- indices.headOption ) yield {
        ...
        ForallRightRule( premise, quantifiedFormula )
      }
    } else
      None
  }
