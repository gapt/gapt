...
object Projections extends at.logic.gapt.utils.logging.Logger {
   ...
   */
  def calculate_child_cut_ecs[T](connector: OccConnector[T],
                                 pm: (LKskProof, Sequent[Boolean]),
                                 default : Boolean): Sequent[Boolean] = {
   ...
   * @return the cut ancestorship sequent for the parent of pm._1
   */
  def calculate_child_cut_ecs[T](connector1: OccConnector[T],
  ...
  def handleContractionRule[Side <: SequentIndex]( proof: LKskProof, p: LKskProof,
                                                   ...
                                                   pred:        HOLFormula => Boolean )
                                                 ( implicit cut_ancs: Sequent[Boolean] ): Set[(LKskProof, Sequent[Boolean])] = {
