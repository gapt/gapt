...
class ceres_omega {
  ...
  /* Bug: This code assumes that we can delete the labels while constructing the proof, not only at the cut. Because of that,
     we might contract the wrong formulas. If the Ral proof comes from a first-order import, it does not matter. A CNF or
     skolemization step introduced at a leaf might not be applicable if we trace the wrong introducing axiom.
    */
  //TODO: fix this bug
   ...
   */
  def apply( projections: Set[(LKskProof, Sequent[Boolean])],
             ralproof: RalProof,
             es: LabelledSequent,
             struct: Struct[Label] ): ( LKskProof, Sequent[Boolean] ) =
    ralproof match {
...
//          require( ( nclause.occurrences diff scontr.root.occurrences ).isEmpty )
    ...
    case RalPara( parent1, p1occ, parent2, p2occ @ Ant( _ ), pos, flipped ) =>
      ...
      val eqn = findAuxInAntecedent(eqlformula , lkparent1.conclusion, Seq(), )
    ...
    case RalPara( parent1, p1occ, parent2, p2occ @ Suc( _ ), pos, flipped ) =>
      val root = ralproof.formulas
      val principial = ralproof.mainFormulas.head._2
      val ( lkparent1, clause1 ) = ceres_omega( projections, parent1, es, struct )
      val ( lkparent2, clause2 ) = ceres_omega( projections, parent2, es, struct )
