...
package at.logic.gapt.provers.atp.commands.base
 ...
 */
abstract class Command[V <: OccSequent] {
...
abstract class DataCommand[V <: OccSequent] extends Command[V] with Function2[State, Any, Iterable[Tuple2[State, Any]]]
...
abstract class InitialCommand[V <: OccSequent] extends Command[V] with Function1[State, Iterable[Tuple2[State, Any]]]
abstract class ResultCommand[V <: OccSequent] extends Command[V] with Function2[State, Any, Option[ResolutionProof[V]]]

case class BranchCommand[V <: OccSequent]( commands: Iterable[Stream[Command[V]]] ) extends Command[V]
// prepend commands in data to current commands
case class PrependCommand[V <: OccSequent]( commands: Iterable[Command[V]] ) extends Command[V]
// prepend commands in data to current commands
case class PrependOnCondCommand[V <: OccSequent]( cond: Function2[State, Any, Boolean], commands: Iterable[Command[V]] ) extends Command[V]
// set the stream given as data as the stream in the next configuration
case class SetStreamCommand[V <: OccSequent]() extends Command[V]
// this command store the configuration in state and starts a new state with the given commands in data
case class SpawnCommand[V <: OccSequent]() extends Command[V]
...
case class RestoreCommand[V <: OccSequent]( commands: Iterable[Command[V]] ) extends Command[V]
