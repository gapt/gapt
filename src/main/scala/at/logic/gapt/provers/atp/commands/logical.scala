...
// using the same state, therefore deterministic
abstract class DeterministicMacroCommand[V <: Sequent]( init: InitialCommand[V], datas: Iterable[DataCommand[V]], result: ResultCommand[V] ) extends Command[V] with Function1[State, Tuple2[Iterable[Tuple2[State, Option[ResolutionProof[V]]]], Boolean]] {
