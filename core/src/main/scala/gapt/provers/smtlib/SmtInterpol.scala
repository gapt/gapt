...
class SmtInterpol(
    val logic: String,
    override val treatUnknownAsSat: Boolean = false
) extends IncrementalProver {
...
class SmtInterpolSession(val script: Script) extends SessionRunner {
  ...
  private def declare[T](n0: String, t: T, ns: mutable.Map[T, String], ins: mutable.Map[String, T], f: String => Unit): Unit =
    try {
