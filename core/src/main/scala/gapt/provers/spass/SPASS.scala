...
import gapt.proofs.resolution.{AvatarNegNonGroundComp, AvatarNonGroundComp, ResolutionProof, resolutionProofsAreReplaceable}
...
class SPASS extends ResolutionProver with ExternalProgram {
  ...
  def cls2dfg(cls: FOLClause): String = {
    val cls_ = FOLSubstitution(freeVariables(cls).zipWithIndex.map { case (v, i) => v -> FOLVar(s"X$i") })(cls)
    s"formula(${expr2dfg(universalClosure(cls_.toDisjunction))})."
  }

  override def getResolutionProof(clauses: Iterable[HOLClause])(implicit ctx: Maybe[MutableContext]): Option[ResolutionProof] = scala.util.boundary {
    renameConstantsToFi.wrap(clauses.toSeq)((renaming, cnf: Seq[HOLClause]) => {
      if (cnf isEmpty) scala.util.boundary.break(None) // SPASS doesn't like empty input
