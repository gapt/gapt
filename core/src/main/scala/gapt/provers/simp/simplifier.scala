      ...
      condInsts match {
        ...
        case (c, Ant(_)) +: cs =>
          simp.prove(c) match {
            ...
            case Some(pc) =>
              procConds(
                if (!p.endSequent.antecedent.contains(c)) p
                else ContractionMacroRule(CutRule(pc, p, c)),
                cs
              )
          }
        case (c, Suc(_)) +: cs =>
          simp.refute(c) match {
            ...
            case Some(pc) =>
              procConds(
                if (!p.endSequent.succedent.contains(c)) p
                else ContractionMacroRule(CutRule(p, pc, c)),
                cs
              )
          }
      }
...
case object QPropSimpProc extends SimpProc {
  ...
  override def simpIff(f0: Formula, polarity: Polarity)(implicit simp: Simplifier): SimpIffResult = {
    ...
    if (f == g) SimpIffResult.Refl(f, polarity)
    else {
      ...
      val Right(lk) = new ExpansionProofToLK(cl =>
        cl.succedent.collectFirst { case Eq(s, t) if s == t => ReflexivityAxiom(t) }
      )(ExpansionProof(expansion)): @unchecked
...
case class Simplifier(lemmas: Seq[SimpProc]) {
  ...
  def simpIff(f: Formula, pol: Polarity): SimpIffResult = {
    val ress =
      lemmas.view.map(_.simpIff(f, pol)).find(!_.isInstanceOf[SimpIffResult.Refl]).getOrElse(simpEq(f, pol) | congrIff(f, pol)) match {
  ...
  def congrIff(f: Formula, pol: Polarity): SimpIffResult = {
    ...
    def congr2(res1: SimpIffResult, res2: SimpIffResult, frm: (Formula, Formula) => Formula, prf: (Formula, Formula, LKProof, Formula, Formula, LKProof) => LKProof): SimpIffResult =
      (res1, res2) match {
    ...
    val res = f match {
      ...
      case And(a, b) =>
        congr2(simpIff(a, pol), simpIff(b, pol), And(_, _), (l1, r1, p1, l2, r2, p2) => AndLeftRule(AndRightRule(p1, p2, r1 & r2), l1 & l2))
      case Or(a, b) =>
        congr2(simpIff(a, pol), simpIff(b, pol), Or(_, _), (l1, r1, p1, l2, r2, p2) => OrRightRule(OrLeftRule(p1, p2, l1 | l2), r1 | r2))
      case Imp(a, b) =>
        congr2(simpIff(a, !pol), simpIff(b, pol), Imp(_, _), (l1, r1, p1, l2, r2, p2) => ImpRightRule(ImpLeftRule(p1, p2, r1 --> l2), l1 --> r2))
      ...
      case All(x, a) =>
        congr1(simpIff(a, pol), All(x, _), (l, r, p) => ForallRightRule(ForallLeftRule(p, All(x, l), x), All(x, r), x))
      case Ex(x, a) =>
        congr1(simpIff(a, pol), Ex(x, _), (l, r, p) => ExistsLeftRule(ExistsRightRule(p, Ex(x, r), x), Ex(x, l), x))
  ...
  def simpEq(formula: Formula, pol: Polarity): SimpIffResult =
    formula match {
      case a: Atom => simpEq(a, pol)
      case _       => SimpIffResult.Refl(formula, pol)
    }
  ...
  private[simp] def project(proj: SimpLemmaProjection, j: Int = 0, x: Var = FOLVar("x")): SimpLemmaProjection = {
    val (newProof, conn) = new LKVisitor[Option[SequentIndex]] {
      ...
      override def visitLogicalAxiom(proof: LogicalAxiom, idx: Option[SequentIndex]): (LKProof, SequentConnector) =
        if (idx.isEmpty) super.visitLogicalAxiom(proof, idx)
        else
...
case class SimpTactic(
...
)(implicit ctx: Context) extends Tactical1[Unit] {
  ...
  private def mkSimpLemmas(goal: OpenAssumption): Seq[SimpProc] = {
    ...
    sls ++= ctx.get[Attributes].lemmasWith("simp").filterNot(excludedLemmasList).flatMap(SimpLemmas.collectFromLemma)
        ...
        val newSeqProof =
          WeakeningMacroRule(
            if (newSeqF.isTaut)
              ...
              }.getOrElse(OpenAssumption(newSeq)),
            newSeqF
          )
