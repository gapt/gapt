...
object ClauseFeatureVec {
  ...
  def apply(cls: Sequent[TermFeatureVec]): ClauseFeatureVec =
    ClauseFeatureVec(cls.antecedent.size, cls.succedent.size, TermFeatureVec.union(cls.antecedent), TermFeatureVec.union(cls.succedent))
}

object fastSubsumption {
  def apply(c1: HOLSequent, c2: HOLSequent, fv1: ClauseFeatureVec, fv2: ClauseFeatureVec, lfv1: Sequent[TermFeatureVec], lfv2: Sequent[TermFeatureVec]): Option[Substitution] = {
    if (!(fv1 <= fv2)) return None
    if (c1.sizes == (1, 0)) {
      val l1 = c1.antecedent.head
      val lf1 = lfv1.antecedent.head
      return c2.antecedent.lazyZip(lfv2.antecedent).view.filter(lf1 <= _._2).map(_._1).flatMap(syntacticMatching(l1, _)).headOption
    }
    if (c1.sizes == (0, 1)) {
