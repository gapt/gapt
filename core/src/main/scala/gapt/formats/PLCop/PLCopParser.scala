...
class PLCopParserException( msg: String ) extends Exception( msg: String )
class PLCopNoMatchException( msg: String ) extends Exception( msg: String )
class PLCopNoLeanPredException( msg: String ) extends Exception( msg: String )
class PLCopLeanPredWrongArityException( msg: String ) extends Exception( msg: String )

object PLCopParser extends RegexParsers with PackratParsers {
  ...
  def getExpansionProof( reader: Reader ): Option[ExpansionSequent] = {
    parseAll( expansionSequent, reader ) match {
      ...
      case Failure( msg, next ) =>
        throw new PLCopParserException( "PLCop parsing: syntax failure " + msg + nLine + "at line " + next.pos.line + " and column " + next.pos.column )
      case Error( msg, next ) =>
        throw new PLCopParserException( "PLCop parsing: syntax error " + msg + nLine + "at line " + next.pos.line + " and column " + next.pos.column )
    }
  }

  // Restricted definitional clausal form (implemented by PLCop)
  // Takes a formula in NNF and returns a list of clauses in DNF (possibly with
  // introduced definitions)
  // (reverse engineering PLCop)
      ...
      case Or( f1, f2 ) => {
        if ( inConj ) {
          ...
          if ( lean_preds.isEmpty ) {
            throw new PLCopNoLeanPredException( "Formula: " + f )
          }
          if ( candidates.isEmpty ) {
            throw new PLCopLeanPredWrongArityException( "Formula: " + f + " Candidates: " + lean_preds + " Arity: " + vars.length )
          }

          // Trusting that we have the same order as PLCop
              ...
              val subs = lean_preds match {
                case Nil =>
                  ...
                  matchClauses( f_dnf, lean_clauses ) match {
                    case Some( s ) => s
                    case None => throw new PLCopNoMatchException( "PLCop parsing: formula " + f_dnf +
                ...
                case _ :: _ =>
                  ...
                  matchClauses( f_clausified, lean_clauses ) match {
                    case Some( s ) => s
                    case None => throw new PLCopNoMatchException( "PLCop parsing: formula " + f_clausified +
  ...
  def clauses: Parser[( Int, FOLFormula, String )] = language ~ "(" ~> integer ~ ", plain," ~ clause ~ ", clausify(" ~ name <~ "))." ^^ {
    ...
    // Equality theory added by PLCop
  ...
  def reduction: Parser[Option[( Int, List[FOLVar], List[FOLTerm] )]] = "reduction('" ~> ReducInteger <~ "')" ^^ { case _ => None }
  ...
  // These are introduced by PLCop's (restricted) definitional clausal form translation
  ...
  lazy val not_eq: PackratParser[FOLFormula] = term ~ "!=" ~ term ^^ {
    case t1 ~ _ ~ t2 => Neg( FOLAtom( "=", List( t1, t2 ) ) )
  }
  ...
  def ReducInteger: Parser[Int] = """((\d+\.)*(\d+))""".r ^^ { case s => s.split( "\\." )( 0 ).toInt }
