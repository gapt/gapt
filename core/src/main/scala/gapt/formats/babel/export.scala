...
class BabelExporter( unicode: Boolean, sig: BabelSignature, omitTypes: Boolean = false ) {
  ...
  def show(
    ...
    expr match {
      ...
      case Abs( v @ Var( vn, vt ), e ) =>
        ...
        ( Parenable( Precedence.lam, ( if ( unicode ) "λ" else "^" ) <> v_ </> e_.inPrec( Precedence.lam ) ),
  ...
  def showApps(
    ...
    if ( retTyKnown || omitTypes ) ( call, t3 ) else
      ( Parenable( Precedence.typeAnnot, call.inPrec( Precedence.typeAnnot ) <> ":" </>
        show( expr.ty, false ) ), t3 )
  }
