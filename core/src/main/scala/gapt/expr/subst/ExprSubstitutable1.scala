...
trait ExprSubstitutable1 {
  ...
  protected def applySub(sub: Substitution, t: Expr): Expr =
    if (sub.isIdentity) t
      ...
      val sub1 = if (sub.typeMap.isEmpty) sub
      else {
        Substitution(
  ...
  private def go(sub: Substitution, t: Expr): Expr = t match {
    ...
    case c @ Const(x, ty, ps) =>
      if (sub.typeMap.isEmpty) c
      else
        Const(x, SubstitutableTy.applySubstitution(sub, ty), ps.map(SubstitutableTy.applySubstitution(sub, _)))
