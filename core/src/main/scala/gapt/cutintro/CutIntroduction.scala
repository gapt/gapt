...
import gapt.expr.formula.fol.isPrenexSigma1
...
object MaxSATMethod {
  def apply(nonTerminalLengths: Int*): MaxSATMethod =
    MaxSATMethod(bestAvailableMaxSatSolver, nonTerminalLengths*)
}
...
object CutIntroduction {
  ...
  def apply(
    ...
    if (useInterpolation && !inputProof.backgroundTheory.hasEquality)
      ...
      compressToSolutionStructure(inputProof, method, useInterpolation).map(constructLKProof(_, inputProof.backgroundTheory))
  ...
  def computeCanonicalSolution(sehs: SchematicExtendedHerbrandSequent): List[FOLFormula] = {
    ...
    val esInstancesPerCut = esInstances.map(identity, -_).elements.groupBy { freeVariables(_).collect(eigenVarIdx).union(Set(sehs.eigenVariables.size)).min }
  ...
  def buildProofWithCut(solStruct: SolutionStructure, prover: Prover): LKProof = {
    ...
    def insertProofOfSolutionCondition(i: Int) = {
      ...
      insert(prover.getLKProof(solCond).getOrElse(throw new UnprovableException(s"Cannot prove solution condition ${i + 1}", solCond)))
    }
    ...
    for (((evs, ss), i) <- solStruct.sehs.ss.zipWithIndex.reverse) {
      ...
      state += focus(1)
      for (inst <- ss) state += allL(s"cut$i", inst*)
