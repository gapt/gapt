    ...
    val result = p match {
      ...
      case p: EqualityRule =>
        ...
        Eql(
          ...
          Bound1(
            aux,
            go(p.subProof, p.getSequentConnector.parents(hyps).map(_.head).updated(p.aux, aux).updated(p.eq, eq))
          )
        )
            ...
            case (c, i) =>
              ...
              IndCase(
                ...
                BoundN(
                  ...
                  go(
                    c.proof,
                    p.occConnectors(i).parent(hyps).updated((c.conclusion -> goal) +: c.hypotheses.zip(ihs))
                  )
                )
              )
          }
...
object LKtToLK {
  ...
  private def down(r: LKProof, s1: Sequent[Hyp], s2: Sequent[Hyp], main: Hyp): (LKProof, Sequent[Hyp]) =
    r -> r.occConnectors.head.children(s1).zip(r.occConnectors(1).children(s2)).map { case (cs1, cs2) => (cs1.view ++ cs2).head }.updated(r.mainIndices.head, main)
  ...
  def withMap(p: LKt, lctx: LocalCtx): (LKProof, Sequent[Hyp]) =
    contract(p match {
      case Cut(_, q1, q2) =>
        ...
        r -> r.getLeftSequentConnector.children(s1).zip(r.getRightSequentConnector.children(s2)).map { case (cs1, cs2) => (cs1 ++ cs2).head }
