...
object ResolutionToLKProof {
      ...
      contract(
        ...
        p match {
          ...
          case p @ Defn(defConst, defExpr) =>
            ...
            ProofBuilder.c(LogicalAxiom(phi)).u(ConversionLeftRule(_, Ant(0), defAtom)).u(ImpRightRule(_, Ant(0), Suc(0))).c(LogicalAxiom(phi)).u(ConversionRightRule(_, Suc(0), defAtom)).u(ImpRightRule(_, Ant(0), Suc(0))).b(AndRightRule(_, Suc(0), _, Suc(0))).u(ForallRightBlock(_, p.definitionFormula, p.vars)).qed
  ...
  def reduceAxiom(proof: LKProof, idx: SequentIndex)(func: Formula => LKProof): LKProof =
    new LKVisitor[Sequent[Boolean]] {
      ...
      val connToInsert = SequentConnector(
        proofToInsert.endSequent,
        formula +: Sequent() :+ formula,
        for (i <- proofToInsert.endSequent.indicesSequent)
          yield
            if (proofToInsert.endSequent(i) == formula)
      ...
      override def visitContractionLeft(proof: ContractionLeftRule, isAncestor: Sequent[Boolean]): (LKProof, SequentConnector) = scala.util.boundary {
  ...
  def mkSymmProof(t: Expr, s: Expr): LKProof =
    ProofBuilder.c(ReflexivityAxiom(t)).u(WeakeningLeftRule(_, Eq(t, s))).u(EqualityRightRule(_, Ant(0), Suc(0), Eq(s, t))).qed
}
