...
object eliminateCutsET {
    ...
    while (true) {
      ...
      cuts.sortBy(numCutInsts).view.flatMap {
        ...
        case cut @ ETCut.Cut(cut1, cut2) =>
          singleStep(cut1, cut2, cuts.filterNot(_ == cut), epwc.nonCutPart, freeVariablesET.includingEigenVariables(epwc), epwc.dependencyRelation)
      }.headOption match {
  ...
  private def singleStep(cut1: ExpansionTree, cut2: ExpansionTree, rest: Seq[ETCut.Cut], expansionSequent: ExpansionSequent, freeVars: Set[Var], dependencyRelation: Set[(Var, Var)]): Option[(Seq[ETCut.Cut], ExpansionSequent)] = {
    ...
    def quantifiedCut(
        ...
        eigenVariables: Seq[Var],
        child: ExpansionTree
    ): (Seq[ETCut.Cut], ExpansionSequent) = {
