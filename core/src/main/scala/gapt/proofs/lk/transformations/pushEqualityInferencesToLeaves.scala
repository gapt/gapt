    ...
    equality.subProof match {
      ...
      case cut @ CutRule(_, _, _, _) =>
        ...
        val newSubProof = CutRule(
          newLeftSubProof,
          leftConnector.child(cut.aux1),
          newRightSubProof,
          rightConnector.child(cut.aux2)
        )
      ...
      case neg @ NegRightRule(_, _) =>
        ...
        val (Seq((newSubProof, subConnector)), weakeningIntro) = transformations.splitEquality(
          equality,
          (neg.subProof, neg.getSequentConnector, context) :: Nil
        )
      ...
      case and @ AndRightRule(left, aux1, right, aux2) =>
        ...
        val newProof = AndRightRule(
          newLeftSubProof,
          leftConnector.child(aux1),
          newRightSubProof,
          rightConnector.child(aux2)
        )
      ...
      case imp @ ImpRightRule(subProof, _, _) =>
        if (imp.mainIndices.head != equality.aux) {
          ...
          val newEquality = EqualityRightRule(
            subProof,
            impConnector.parent(equality.eq),
            impConnector.parent(equality.aux),
            equality.replacementContext
          )
          val newConnector = newEquality.getSequentConnector
      ...
      case exists @ ExistsLeftRule(subProof, _, eigenVariable, quantifiedVariable) =>
        ...
        Some(
          ExistsLeftRule(
            newEquality,
            newEquality.getSequentConnector.child(exists.aux),
            eigenVariable,
            quantifiedVariable
          ),
          false
      ...
      case ind @ InductionRule(_, _, _) if ind.mainIndices.head != equality.aux =>
        val (splitCases, weakeningIntro) = splitEquality(
          equality,
          ind.cases.zip(ind.occConnectors).map {
        ...
        val newIndCases = newSubProofs.map {
          case ((subProof, connector), indCase) =>
            InductionCase(
              ...
              indCase.eigenVars,
              connector.child(indCase.conclusion)
            )
        }
    ...
    equality.subProof match {
      ...
      case or @ OrLeftRule(_, _, _, _) =>
        ...
        val newProof = OrLeftRule(
          newLeftProof,
          leftConnector.child(or.aux1),
          newRightProof,
          rightConnector.child(or.aux2)
        )
      ...
      case or @ OrRightRule(_, _, _) =>
        ...
        Some(
          OrRightRule(
            newEqInf,
            newEqInf.getSequentConnector.child(or.aux1),
            newEqInf.getSequentConnector.child(or.aux2)
          ),
          false
        )
      ...
      case and @ AndLeftRule(_, _, _) =>
        ...
        if (and.mainIndices.head == equality.aux) {
          ...
          Some(
            AndLeftRule(
              newEqInf,
              newEqInf.getSequentConnector.child(and.aux1),
              newEqInf.getSequentConnector.child(and.aux2)
            ),
            false
          )
        }
      ...
      case imp @ ImpLeftRule(_, _, _, _) =>
        ...
        val newProof = ImpLeftRule(
          newLeftProof,
          leftConnector.child(imp.aux1),
          newRightProof,
          rightConnector.child(imp.aux2)
        )
        Some(ContractionMacroRule(newProof, equality.conclusion, false), weakeningIntro)
      ...
      case imp @ ImpRightRule(_, _, _) =>
        ...
        Some(
          ImpRightRule(
            newEqInf,
            newEqInf.getSequentConnector.child(imp.aux1),
            newEqInf.getSequentConnector.child(imp.aux2)
          ),
          false
        )
      ...
      case exists @ ExistsLeftRule(_, _, _, _) =>
        ...
        Some(
          ExistsLeftRule(
            newSubProof,
            subConnector.child(exists.aux),
            exists.eigenVariable,
            exists.quantifiedVariable
          ),
          false
        )
      ...
      case exists @ ExistsRightRule(_, _, _, _, _) =>
        val (Seq((newSubProof, subConnector)), _) = transformations.splitEquality(
          equality,
          (exists.subProof, exists.getSequentConnector, equality.replacementContext) :: Nil
        )
      ...
      case all @ ForallLeftRule(_, _, _, _, _) =>
        ...
        val (Seq((newSubProof, subConnector)), _) = transformations.splitEquality(
          equality,
          (all.subProof, all.getSequentConnector, context) :: Nil
        )
      ...
      case all @ ForallRightRule(_, _, _, _) =>
        val (Seq((newSubProof, newConnector)), _) = transformations.splitEquality(
          equality,
          (all.subProof, all.getSequentConnector, equality.replacementContext) :: Nil
        )
        Some(
          ForallRightRule(
            newSubProof,
            newConnector.child(all.aux),
            all.eigenVariable,
            all.quantifiedVariable
          ),
          false
        )
...
private object splitEquality {
  ...
  private def createEquality(
      subProof: LKProof,
      equation: SequentIndex,
      auxiliary: SequentIndex,
