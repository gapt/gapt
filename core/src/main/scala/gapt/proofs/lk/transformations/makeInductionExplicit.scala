  ...
  private object explicitInductionVisitor extends LKVisitor[Unit] {
    ...
    override def visitInduction(proof: InductionRule, otherArg: Unit): (LKProof, SequentConnector) = {
      ...
      val hypP = hyps.reduceLeft((p, hyp) =>
        AndRightRule(p, p.conclusion.indices.last, hyp, hyp.conclusion.indices.last)
      )

      val explicitFOLp =
        ProofBuilder.c(LogicalAxiom(proof.mainFormula)).u(ForallLeftRule(_, Ant(0), All(proof.quant, proof.qfFormula), proof.term)).u(ImpLeftRule(hypP, hypP.conclusion.indices.last, _, Ant(0))).qed
      val explicitHOLp =
