...
private class skolemizeLK(
  ...
  case class Info(
    ...
    def atPosition(pos: Int*): Info = atPosition(HOLPosition(pos*))
    ...
    def instantiateQuantifier(term: Expr) =
      copy(
        generalizedFormulas = generalizedFormulas.collect { case quant @ Quant(_, _, _) => BetaReduction.betaNormalize(instantiate(quant, term)) },
        position = position :+ 1
      )
    ...
    p match {
      ...
      case p @ AndLeftRule(q, a1, a2) =>
        AndLeftRule(
          apply(
            q,
            p.getSequentConnector.parent(info).updated(a1, info(p.mainIndices.head).atPosition(1)).updated(a2, info(p.mainIndices.head).atPosition(2)),
            subst
          ),
          a1,
          a2
      ...
      case p @ AndRightRule(q1, a1, q2, a2) =>
        AndRightRule(
          apply(q1, p.getLeftSequentConnector.parent(info).updated(a1, info(p.mainIndices.head).atPosition(1)), subst),
          a1,
          apply(q2, p.getRightSequentConnector.parent(info).updated(a2, info(p.mainIndices.head).atPosition(2)), subst),
          a2
        )
      ...
      case p: EqualityRule =>
        val subProofNew =
          apply(
            p.subProof,
            p.getSequentConnector.parent(info).updated(p.aux, info(p.auxInConclusion).copy(generalizedFormulas = Seq(p.auxFormula))).updated(p.eq, info(p.eqInConclusion)),
            subst
          )
        if (p.aux.isAnt)
      ...
      case p @ CutRule(q1, a1, q2, a2) =>
        CutRule(
          apply(q1, p.getLeftSequentConnector.parent(info, Info(Seq(p.cutFormula), isCutAnc = true, Seq(), Seq(-1))), subst),
          a1,
          apply(q2, p.getRightSequentConnector.parent(info, Info(Seq(p.cutFormula), isCutAnc = true, Seq(), Seq(-1))), subst),
          a2
        )

      case p @ ConversionLeftRule(q, a, m) =>
        val qNew = apply(q, p.getSequentConnector.parent(info).updated(a, info(p.mainIndices.head).copy(generalizedFormulas = Seq(q.conclusion(a)))), subst)
      ...
      case p @ StrongQuantifierRule(q, a, eigen, quant, pol) if info(p.mainIndices.head).isCutAnc && !skolemizeAboveCuts =>
        val q_ = apply(q, p.occConnectors.head.parent(info).updated(a, info(p.mainIndices.head).instantiateQuantifier(eigen)), subst)
