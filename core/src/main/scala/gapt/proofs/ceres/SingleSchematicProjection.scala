...
object SingleSchematicProjection {

  // This method computes the standard projections according to the original CERES definition.
  def apply( proof: LKProof )( implicit ctx: MutableContext ):Unit =
    apply( proof, proof.endSequent.map( _ => false ), _ => true )(ctx)._2

  def apply( proof: LKProof, pred: Formula => Boolean )( implicit ctx: MutableContext ):Unit =
    apply( proof, proof.endSequent.map( _ => false ), pred )(ctx)._2

  def apply( proof: LKProof, cut_ancs: Sequent[Boolean], pred: Formula => Boolean )( implicit ctx: MutableContext ):(Option[SequentIndex],LKProof) = {
    apply_( proof, cut_ancs, pred )
  }

  def apply_( proof: LKProof, cut_ancs: Sequent[Boolean], pred: Formula => Boolean )( implicit ctx: MutableContext ):(Option[SequentIndex],LKProof) = {
  ...
  def handleBinaryCutAnc(s1:(Option[SequentIndex],LKProof), s2:(Option[SequentIndex],LKProof),
    ...
    if(s1._1.nonEmpty){
      if(s2._1.nonEmpty){
        //  println("one:   " + s1._2.endSequent(s1._1.get)+"\n\n   "+"two:   "+s2._2.endSequent(s2._1.get)+"\n\n   ")
        val proof = AndRightRule(s1._2,s1._2.endSequent(s1._1.get),s2._2,s2._2.endSequent(s2._1.get))
