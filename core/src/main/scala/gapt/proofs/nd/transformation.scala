  ...
  def apply(proof: NDProof, a: Formula): NDProof = {
    ...
    proof match {
      ...
      case ExistsElimRule(leftSubProof, rightSubProof, aux, eigenVariable) =>
        ...
        ExistsElimRule(
          friedman(leftSubProof, a),
          friedman(Substitution(eigenVariable, eig)(rightSubProof), a),
          aux,
          eig
        )
      ...
      case BottomElimRule(subProof, mainFormula) =>
        ...
        NegIntroRule(
          WeakeningRule(
            NegElimRule(
              kolmogorov(subProof),
              NegIntroRule(LogicalAxiom(Bottom()))
            ),
            lessnegated
          ),
          lessnegated
  ...
  private def elimcase(elimproof: NDProof, conclproof: NDProof): NDProof = {
    ...
    NegIntroRule(
      NegElimRule(
        elimproof,
        NegIntroRule(
          NegElimRule(conclproof, LogicalAxiom(lessnegated)),
          Ant(0)
        )
      ),
      lessnegated
