...
package object nd {
  ...
  implicit val ndSubstitutable: ClosedUnderSub[NDProof] = (s, p) =>
    p match {
  ...
  implicit object casesReplaceable extends ClosedUnderReplacement[InductionCase] {
    ...
    override def replace(c: InductionCase, p: PartialFunction[Expr, Expr]): InductionCase =
      InductionCase(TermReplacement(c.proof, p), TermReplacement(c.constructor, p).asInstanceOf[Const], c.hypotheses, TermReplacement(c.eigenVars, p).map(_.asInstanceOf[Var]))
  ...
  implicit object replaceable extends ClosedUnderReplacement[NDProof] {
    def replace(p: NDProof, r: PartialFunction[Expr, Expr]): NDProof = p match {
      ...
      case ForallIntroRule(q, ev, qv) =>
        ForallIntroRule(replace(q, r), TermReplacement(ev, r).asInstanceOf[Var], TermReplacement(qv, r).asInstanceOf[Var])
