 ...
 */
class SequentProofViewer[F, T <: SequentProof[F, T]](name: String, proof: SequentProof[F, T], sequent_element_renderer: F => String)
...
class LKProofViewer(name: String, proof: LKProof)
  ...
  override def viewMenuContents = super.viewMenuContents ++ Seq(
    hideStructuralRulesButton,
    markCutAncestorsButton,
    markNonIntuitionisticInferencesButton,
    new Separator(),
