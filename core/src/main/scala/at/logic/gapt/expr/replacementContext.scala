...
object replacementContext {
   ...
   * @return
   */
  def apply( ty: Ty, exp: LambdaExpression, positions: Iterable[LambdaPosition], terms: LambdaExpression* ): Abs = {
   ...
   * @return
   */
  def apply( ty: Ty, exp: LambdaExpression, positions: Iterable[HOLPosition], terms: LambdaExpression* )( implicit d: DummyImplicit ): Abs = apply( ty, exp, positions map { HOLPosition.toLambdaPosition( exp ) }, terms: _* )
