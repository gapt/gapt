      ...
      val currentProofStruct: Map[CLS, ( Struct, Set[Var] )] =
        ctx.get[ProofDefinitions].components.getOrElse( topSym, Set() ).map {
          case ProofDefinition( placeHolder: Expr, _, assocProof: LKProof ) =>
            ( CLS( placeHolder, theActualConfig ), ( StructCreators.extract( assocProof, theActualConfig )( { _ => true }, ctx ), EigenVariablesLK( assocProof ) ) )
        }.toMap
  ...
  def apply( topSym: CLS, sss: Map[CLS, ( Struct, Set[Var] )], usedNames: Set[Var] = Set[Var]() )( implicit ctx: Context ): Struct = {
    ...
    val renamedStruct = Set( usedNames.foldLeft( ( ( rename.awayFrom( usedNames ), usedNames ), starterStruct._1 ) )(
      ( reClause, nameVar ) => Set[Var]( Var( reClause._1._1.fresh( nameVar.name ), nameVar.ty ) ).map( newVar =>
        ( ( reClause._1._1, reClause._1._2 + newVar ), TermReplacement( reClause._2, Map( ( nameVar, newVar ) ) ) ) ).head ) ).map( x => ( x._1._2, x._2 ) ).head
...
object InstantiateStruct extends StructVisitor[Struct, ( Substitution, Map[CLS, ( Struct, Set[Var] )], Set[Var] )] {
  def apply( theStruct: Struct, sigma: Substitution, sss: Map[CLS, ( Struct, Set[Var] )], usedNames: Set[Var] )( implicit ctx: Context ): Struct = {
    val Transform = StructTransformer[Struct, ( Substitution, Map[CLS, ( Struct, Set[Var] )], Set[Var] )](
      aF, { ( x, y, _ ) => Plus( x, y ) }, EmptyPlusJunction(), { ( x, y, _ ) => Times( x, y ) }, EmptyTimesJunction(), { ( x, _ ) => Dual( x ) }, cF )
    recurse( theStruct, Transform, ( sigma, sss, usedNames ) )
  }
  def aF( f: Formula, info: ( Substitution, Map[CLS, ( Struct, Set[Var] )], Set[Var] ) )( implicit ctx: Context ): Struct = A( info._1( f ) )
