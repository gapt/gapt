...
object eliminateDefinitions {
  ...
  def rec( proof: LKProof ): ( LKProof, mutable.Map[LabelledFormulaOccurrence, LabelledFormulaOccurrence] ) =
    {
      proof match {
        case r @ CutRule( p1, p2, _, a1, a2 ) => {
          ...
          return ( new_proof,
            computeMap( ls1.antecedent ++ ls1.succedent.filter( _ != a1 ), r, new_proof, rec1._2 ) ++
            computeMap( ls2.antecedent.filter( _ != a2 ) ++ ls2.succedent, r, new_proof, rec2._2 ) )
        }
        ...
        case Axiom( so ) => {
          ...
          val ant_occs = ls.antecedent.toList
          val succ_occs = ls.succedent.toList
          ...
          las.antecedent.zip( ant_occs ).foreach( p => { println( p ); map.update( p._2, p._1 ) } )
          las.succedent.zip( succ_occs ).foreach( p => { println( p ); map.update( p._2, p._1 ) } )
          ( a, map )
        }
        ...
        case AndLeft1Rule( p, s, a, m ) => {
          ...
          ( new_proof, computeMap( ls.antecedent ++ ls.succedent, proof, new_proof, new_parent._2 ) )
        }
  ...
  def handleDefinition( r: LKProof, p: LKProof ) = {
    ...
    ls.antecedent.foreach( fo => { println( fo ); println( fo.parents.head ); map.update( fo, premiseMap( fo.parents.head ) ) } )
    ls.succedent.foreach( fo => map.update( fo, premiseMap( fo.parents.head ) ) )
    println( "map" )
  ...
  def handleWeakening(
    new_parent:  ( LKProof, mutable.Map[LabelledFormulaOccurrence, LabelledFormulaOccurrence] ),
    old_parent:  LKProof,
    old_proof:   LKProof,
    ...
    ( new_proof, computeMap( ls.antecedent ++ ls.succedent, old_proof, new_proof, new_parent._2 ) + Tuple2( m, new_proof.prin.head.asInstanceOf[LabelledFormulaOccurrence] ) )
  }
  ...
  def handleContraction(
    ...
    val ls = toLabelledSequent( old_parent.root )
    ( new_proof, computeMap( ls.antecedent ++ ls.succedent, old_proof, new_proof, new_parent._2 ) )
  }
  ...
  def handleEquational( r: BinaryLKProof with AuxiliaryFormulas, p1: LKProof, p2: LKProof, a1: LabelledFormulaOccurrence, a2: LabelledFormulaOccurrence, m: HOLFormula,
    ...
    ( new_proof, computeMap( ls1.antecedent ++ ls1.succedent, r, new_proof, rec1._2 ) ++
      computeMap( ls2.antecedent ++ ls2.succedent, r, new_proof, rec2._2 ) )
  }
  ...
  def computeMap( occs: Seq[LabelledFormulaOccurrence], old_proof: LKProof,
                  new_proof: LKProof, old_map: mutable.Map[LabelledFormulaOccurrence, LabelledFormulaOccurrence] ) =
    {
      ...
      occs.foreach( fo => map.update(
        old_proof.getDescendantInLowerSequent( fo ).get.asInstanceOf[LabelledFormulaOccurrence],
