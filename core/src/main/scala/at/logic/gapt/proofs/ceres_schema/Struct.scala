...
import at.logic.gapt.proofs.lksk.{ LabelledFormulaOccurrence, LabelledOccSequent, UnaryLKskProof }
...
import at.logic.gapt.expr.schema.{ BigAnd, BigOr, BiggerThan, IndexedPredicate, IntVar, IntZero, IntegerTerm, Pred, SchemaFormula, SchemaSubstitution, Succ }
...
import at.logic.gapt.proofs.ceres_schema.RelevantCC
    ...
    case Axiom( so ) => // in case of axioms of the form A :- A with labelled formulas, proceed as in Daniel's PhD thesis
      {
        debug( "0 " + cut_occs + "  " );
        so match {
          case lso @ Sequent( Seq( _: LabelledFormulaOccurrence ), Seq( _: LabelledFormulaOccurrence ) ) =>
            handleLabelledAxiom( lso.asInstanceOf[LabelledOccSequent], cut_occs )
