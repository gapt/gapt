...
trait StructVisitor[Ret,T] {
  final def recurse[Data](struct: Struct[Data],
  ...
  def visitDual[Data](struct: Struct[Data],
                tranform:StructTransformer[Ret,T],
                otherArgs:T ): Ret = {
  ...
  def visitAtomLeaf[Data](f: Formula,
                    tranform:StructTransformer[Ret,T],
                    otherArgs:T ): Ret ={
  ...
  def visitPlus[Data](struct1: Struct[Data],
                struct2: Struct[Data],
                tranform:StructTransformer[Ret,T],
                otherArgs:T ): Ret = tranform.pF(recurse(struct1,tranform, otherArgs),
    recurse(struct2,tranform, otherArgs),
                                                 otherArgs)
  def visitTimes[Data](struct1: Struct[Data],
                 struct2: Struct[Data],
                 tranform:StructTransformer[Ret,T],
                otherArgs:T ): Ret = tranform.tF(recurse(struct1,tranform, otherArgs),
 ...
 case class StructTransformer[Ret,T](aF: (Formula,T) => Ret,
                                   pF: (Ret, Ret,T)=> Ret,
                                   pD: Ret,
                                   tF: (Ret ,Ret,T)=> Ret,
                                   tD: Ret, dF:(Ret,T) => Ret,
