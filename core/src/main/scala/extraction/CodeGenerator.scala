    ...
    "def app[A,B](f: A => B)(arg: A): B = f(arg)\n" +
      context.constants.filter {
        ...
        case Const( "bar", _, params ) =>
          ...
          s"def bar[$a,$b,$c](p1: Boolean)(p2: $a => $c)(p3: $b => $c): $c = {???}"
...
if(p3 == 0) {
  ...
  private def toScalaName( c: Const ): String = {
    c match {
      ...
      case Const( "=", _, _ )  => "eq"
      case Const( "inl", _, params ) =>
        s"inl[${toScalaType( params( 0 ) )}, ${toScalaType( params( 1 ) )}]"
      case Const( "inr", _, params ) =>
        s"inr[${toScalaType( params( 0 ) )}, ${toScalaType( params( 1 ) )}]"
      case _ => c.name
    }
  }
  ...
  private def translateLambda( e: Expr )( implicit ctx: Context ): String = {
    e match {
      case App( e1, e2 ) =>
        ...
        */
        s"app(${translateLambda( e1 )})(${translateLambda( e2 )})"
      case Abs( v, e ) =>
