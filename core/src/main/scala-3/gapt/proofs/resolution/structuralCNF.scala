...
object structuralCNF {
  def apply(
      ...
      implicit ctx: MutableContext = MutableContext.guess(endSequent)
  ): Set[ResolutionProof] = {
    if (!propositional)
...
class Clausifier(
  ...
  def expand(p: ResolutionProof): Unit = scala.util.boundary {
    val p_Option = p.conclusion.zipWithIndex.elements.collectFirst {
      ...
      case (Top(), i: Suc)    => scala.util.boundary.break(())
      case (Bottom(), i: Ant) => scala.util.boundary.break(())
