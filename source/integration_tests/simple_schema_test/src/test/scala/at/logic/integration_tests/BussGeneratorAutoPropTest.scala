...
/* ------ for Stefan :
Uncomment for each instance and look at prooftol
 */

//      val fs = BussTautology(2)
      val fs = BussTautology(3)
//      val fs = BussTautology(4)
//      val bussGen = Autoprop(fs)
//      Main.display("Buss Generator", bussGen);
//      while(true){}
      Success()
    }
  }
