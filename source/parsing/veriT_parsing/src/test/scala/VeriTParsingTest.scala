...
class VeriTParsingTest extends SpecificationWithJUnit {
  ...
  "The veriT parser" should {
    "parse correctly the simplest proof of the database" in {
      val formulas = VeriTParser.getExpansionProof("target/test-classes/test0.verit")
      formulas.get._1 must haveSize(2)
    }

    "parse correctly a more complicated example" in {
      val formulas = VeriTParser.getExpansionProof("target/test-classes/test1.verit")
      // Only 3 expansion trees: input, eq_transitive (with a million
      // instances!) and eq_symmetry (with hundreds of instances)
      formulas.get._1 must haveSize(3)
    }

    "parse correctly an even more complicated example" in {
      val formulas = VeriTParser.getExpansionProof("target/test-classes/test2.verit")
      formulas.get._1 must haveSize(4)
    }
    ...
    "parse correctly an example from QG-classification" in {
      ...
      formulas.get._1 must haveSize(5)
    }
    ...
    "parse correctly a different eq_congruent 2" in {
      val formulas = VeriTParser.getExpansionProof("target/test-classes/test4.verit")
      formulas.get._1 must haveSize(11)
    }
  }
}
