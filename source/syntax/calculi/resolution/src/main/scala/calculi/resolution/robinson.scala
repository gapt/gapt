...
package robinson {
  ...
  object Resolution {
    def apply[T <: LambdaExpression](p1: ResolutionProof[Clause], p2: ResolutionProof[Clause], i: Int, j: Int, sub: Substitution[T]): ResolutionProof[Clause] = {
    ...
    def unapply[T <: LambdaExpression](proof: ResolutionProof[Clause] with AppliedSubstitution[T]) = if (proof.rule == ResolutionType) {
  ...
  object Paramodulation {
    def apply[T <: LambdaExpression](p1: ResolutionProof[Clause], p2: ResolutionProof[Clause], i: Int, j: Int, newLiteral: HOLFormula, sub: Substitution[T]): ResolutionProof[Clause] = {
    ...
    // compose two clauses on all elements except with the index given and apply sub on all terms
    private def createClause[T <: LambdaExpression](c1: Clause, c2: Clause, i: Int, j: Int, newLiteral: HOLFormula, sub: Substitution[T]) = {
      val (neg1,pos1) = if (i < c1.negative.size)
          (removeAtIndex(c1.negative, i), c1.positive)
        else (c1.negative, removeAtIndex(c1.positive, i - c1.negative.size))
      val (neg2,pos2) = if (j < c2.negative.size)
  ...
  object Variant {
    ...
    def unapply[T <: LambdaExpression](proof: ResolutionProof[Clause] with AppliedSubstitution[T]) = if (proof.rule == VariantType) {
