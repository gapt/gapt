...
trait TokenToLKConverter {
  ...
  *  to completeProof have to be ordered s.t. dependent proofs are already contained in the map.*/
  ...
  val axformula = Atom(HOLConst(ConstantStringSymbol("AX"), To()),Nil)
  ...
  def handleEQAxiom(current_proof: List[LKProof], ruletype: String, fs: FSequent, auxterm: Option[LambdaAST],
    ...
    val candidates = axs.flatMap( s => {
      ...
      val r1 = NaiveIncompleteMatchingAlgorithm.holMatch(ax, auxf)(Nil) match {
        ...
        case Some(sub) =>
          val sub2 = Substitution[HOLExpression](sub.map.filter(x => x._1 != x._2))
  ...
  def handleInstAxiom(current_proof: List[LKProof], ruletype: String, fs: FSequent, auxterm: Option[LambdaAST], subterm : List[(ast.Var,LambdaAST)],
    ...
    val candidates = axs.flatMap( s => {
      ...
      val r1 = NaiveIncompleteMatchingAlgorithm.holMatch(ax, auxf)(Nil) match {
        case Some(sub) if sub(ax) syntaxEquals(auxf) => (name,ax1,sub)::Nil
        case Some(sub2)  =>
          val sub = Substitution[HOLExpression](sub2.map.filterNot(x => x._1 == x._2))
          if (sub(ax) syntaxEquals(auxf) ) {
            (name,ax1,sub)::Nil
          } else {
...
//        case Some(sub) => (ax,sub)::Nil
  ...
  private def f(s:Substitution[_]) : String = {
    s.map.map( x => f(x._1.asInstanceOf[HOLExpression])  + " -> " + f(x._2.asInstanceOf[HOLExpression])  ).mkString("{",",","}")
  }
  ...
  def getAxiomLookupProof(name : HOLFormula, axiom : HOLFormula, instance: HOLFormula,
                          axiomconj : HOLFormula, axiomproof : LKProof, sub: Substitution[HOLExpression]) : (HOLFormula,LKProof) = {
    axiomconj match {
  ...
  def proveInstance(axiom: HOLFormula, instance:HOLFormula, sub: Substitution[HOLExpression]) : LKProof = {
    //val (qs,body) = stripUniversalQuantifiers(axiom)
    proveInstance_(axiom,instance,sub, Axiom(List(instance), List(instance)))._2
  }

  def proveInstanceFrom(axiom: HOLFormula, instance:HOLFormula, sub:Substitution[HOLExpression], uproof : LKProof) : LKProof = {
    //val (qs,body) = stripUniversalQuantifiers(axiom)
    proveInstance_(axiom,instance,sub, uproof)._2
  }
  def proveInstance_(axiom: HOLFormula, instance:HOLFormula, sub : Substitution[HOLExpression], axiomproof : LKProof) : (HOLFormula,LKProof) = {
//    println("Prove instance with sub "+f(sub))
  ...
  def univclosure(f:HOLFormula) = f.freeVariables.foldRight(f)((v,g) => AllVar(v.asInstanceOf[HOLVar],g))
