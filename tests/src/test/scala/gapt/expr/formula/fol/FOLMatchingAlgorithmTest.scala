...
class FOLMatchingAlgorithmTest extends Specification {
  "FOLMatchingAlgorithm" should {
    ...
    "not match f(x1,x2,x1) with f(x1,x2,x2)" in {
      syntacticMatching(FOLFunction("f", x1, x2, x1), FOLFunction("f", x1, x2, x2)) must beNone
    }

    "not match f(x,g(x)) with f(g(x),g(x))" in {
      syntacticMatching(FOLFunction("f", x, FOLFunction("g", x)), FOLFunction("f", FOLFunction("g", x), FOLFunction("g", x))) must beNone
    }
  ...
  "many sorted" in {
    syntacticMatching(
      le"x:nat",
      le"(#v(α: nat) + #v(α: nat): nat) + (s(0:nat): nat) = s(#v(α: nat) + #v(α: nat) + 0)"
    ) must beNone
  }
  ...
  "induction axioms" in {
    syntacticMatching(
      ...
      le"∀X X(#c(nil{?a_0}: list ?a_0))"
    ) must beSome
    syntacticMatching(
      le"#c(nil{?a}: list ?a)",
      le"#c(nil{?a_0}: list ?a_0)"
    ) must beSome
    syntacticMatching(
      le"∀X (X(#c(nil{?a}: list ?a)) ∧ ∀x0 ∀x1 (X(x1) → X(cons(x0:?a, x1))) → ∀x X(x))",
      le"∀X (X(#c(nil{?a_0}: list ?a_0)) ∧ ∀x0 ∀x1 (X(x1) → X(cons(x0:?a_0, x1))) → ∀x X(x))"
    ) must beSome
  }
}
