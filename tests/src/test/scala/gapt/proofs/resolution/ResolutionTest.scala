...
class ResolutionTest extends Specification with SatMatchers {
  ...
  "Resolution" in {
    Resolution(
      Input(Clause() :+ hoa"f(x)"),
      Suc(0),
      Input(hoa"f(x)" +: Clause()),
      Ant(0)
    ...
    Resolution(
      Input(hoa"a" +: Clause() :+ hoa"f(x)" :+ hoa"b"),
      Suc(0),
      Input(hoa"c" +: hoa"f(x)" +: Clause() :+ hoa"d"),
      Ant(1)
  ...
  "Paramod" in {
    Paramod(
      Input(Clause() :+ hoa"f(c) = g(d)"),
      Suc(0),
      true,
      Input(hoa"a" +: Clause() :+ hoa"p(f(c), f(c))"),
      Suc(0),
  ...
  "Splitting" in {
    ...
    val c1 = ProofBuilder.c(in).u(ImpL1(_, Ant(0))).u(AllR(_, Suc(0), hov"x")).u(AllR(_, Suc(0), hov"y")).u(OrR(_, Suc(0))).qed
    val c2 = ProofBuilder.c(in).u(ImpL2(_, Ant(0))).u(OrL1(_, Ant(0))).qed
    val c3 = ProofBuilder.c(in).u(ImpL2(_, Ant(0))).u(OrL2(_, Ant(0))).qed
