...
class ExpansionProofTest extends Specification with SatMatchers with SequentMatchers {
  ...
  "linear example cut intro" in {
    ...
    val e = ETtoET2( LKToExpansionProof( p ) )
  ...
  "pi2 pigeonhole" in {
    val e = ETtoET2( LKToExpansionProof( Pi2Pigeonhole.proof ) )
  ...
  "tape proof cut elimination" in {
    ...
    val expansion = ETtoET2( LKToExpansionProof( lk ) )
    val cutfree = eliminateCutsET( expansion, 2 )
  ...
  "merge skolem and strong quantifiers" in {
    ...
    merged must beLike { case e if !e.subProofs.exists { case ETMerge( _, _ ) => true case _ => false } => ok }
  }
