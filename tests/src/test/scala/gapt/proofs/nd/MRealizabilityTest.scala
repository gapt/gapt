...
class MRealizabilityTest extends Specification {
  ...
  "mrealizers" in {
    ...
    def bitreeRec(bitreeType: Ty, resultType: Ty) =
      Const(
        "bitreeRec",
        (bitreeType ->: resultType) ->: (bitree(bitreeType) ->: resultType ->: bitree(bitreeType) ->: resultType ->: resultType) ->: bitree(bitreeType) ->: resultType,
        List(bitreeType, resultType)
      )
