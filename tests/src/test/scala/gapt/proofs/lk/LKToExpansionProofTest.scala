...
class LKToExpansionProofTest extends Specification with SatMatchers with SequentMatchers {
  ...
  "The expansion tree extraction" should {
    ...
    "do merge triggering a substitution triggering a merge" in {
      ...
      val p = ProofBuilder.c(ProofLink(hoc"ax: i", hcl"P α, P β :- Q (f α) c, Q (f β) d")).u(ExistsRightRule(_, hof"∃z Q (f α) z", le"c")).u(ExistsRightRule(_, hof"∃z Q (f β) z", le"d")).u(ExistsRightRule(_, hof"∃y ∃z Q y z", le"f α")).u(ExistsRightRule(_, hof"∃y ∃z Q y z", le"f β")).u(ContractionRightRule(_, hof"∃y ∃z Q y z")).u(ExistsLeftRule(_, hof"∃x P x", fov"α")).u(ExistsLeftRule(_, hof"∃x P x", fov"β")).u(ContractionLeftRule(_, hof"∃x P x")).qed
      ...
      E.succedent must_== Seq(ExpansionTree(
        hof"∃y ∃z Q y z",
    ...
    "equation left rule" in {
      val lk = ProofBuilder.c(LogicalAxiom(hof"a=b")).u(WeakeningLeftRule(_, hof"b=c")).u(EqualityLeftRule(_, hof"a=b", hof"b=c", hof"a=c")).qed
      LKToExpansionProof(lk).shallow must_== lk.conclusion
    }
    ...
    "handle atom definitions in top position" in {
      ...
      val p = ProofBuilder.c(LogicalAxiom(fof"x = (x:i)")).u(NegRightRule(_, Ant(0))).u(OrRightRule(_, Suc(0), Suc(1))).u(ConversionRightRule(_, Suc(0), fof"P(x)")).u(OrRightMacroRule(_, fof"P(x)", fof"Q(x)")).qed
    ...
    "handle atom definitions in non-top position" in {
      ...
      val p = ProofBuilder.c(LogicalAxiom(fof"x = (x:i)")).u(NegRightRule(_, Ant(0))).u(OrRightRule(_, Suc(0), Suc(1))).u(OrRightMacroRule(_, fof"x = (x:i) ∨ ¬ x = x", fof"Q(x)")).u(ConversionRightRule(_, Suc(0), fof"P(x) ∨ Q(x)")).qed
    ...
    "handle term definitions" in {
      ...
      val p = ProofBuilder.c(LogicalAxiom(fof"f( g x) = f (g x)")).u(NegRightRule(_, Ant(0))).u(OrRightRule(_, Suc(0), Suc(1))).u(ConversionRightRule(_, Suc(0), fof"h x = f (g x) ∨ ¬ f (g x) = f (g x)")).qed
    ...
    "work on a simple example of a term definition of type o" in {
      ...
      val p = ProofBuilder.c(LogicalAxiom(hof"S(Q & R)")).u(ConversionRightRule(_, Suc(0), hof"S P")).qed
    ...
    "fail on double negation definition example" in {
      ...
      val p = ProofBuilder.c(LogicalAxiom(hoa"X")).u(NegLeftRule(_, Suc(0))).u(NegRightRule(_, Ant(0))).u(ImpRightRule(_, Ant(0), Suc(0))).u(ConversionRightRule(_, Suc(0), hof"X -> n (n X)")).qed
    ...
    "lattice with definitions" in {
      ...
      val Right(lk) = ExpansionProofToLK.withTheory(using implicitly)(exp): @unchecked
      ctx.check(exp)
      ctx.check(lk)
