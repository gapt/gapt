...
class LKTest extends Specification {
  ...
  def P(t: FOLTerm) = formula.fol.FOLAtom("P", t)
  ...
  "equality left rule eqInConclusion" in {
    val elr = ProofBuilder.c(LogicalAxiom(hof"a=b")).u(WeakeningLeftRule(_, hof"b=c")).u(EqualityLeftRule(_, hof"a=b", hof"b=c", hof"a=c")).qed.asInstanceOf[EqualityLeftRule]
