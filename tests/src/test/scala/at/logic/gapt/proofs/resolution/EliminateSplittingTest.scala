...
class EliminateSplittingTest extends Specification {
  ...
  "example" in {
    val Some( proofWithSplitting ) = Escargot getRobinsonProof hof"!x p x | !x q x -> !x (p x | q x) & ${BussTautology( 2 ).toImplication}"
  ...
  "counting" in {
    ...
    val Some( proofWithSplitting ) = Escargot getRobinsonProof formula
    val proofWithoutSplitting = eliminateSplitting( proofWithSplitting )
    proofWithoutSplitting.isProof must_== true
  }
