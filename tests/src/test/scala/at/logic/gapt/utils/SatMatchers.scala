...
trait SatMatchers extends OptionMatchers {
  ...
  def beSat =
    beNone ^^ { ( f: HOLFormula ) => new Escargot( splitting = false, equality = false, propositional = true ).getRobinsonProof( f ) } and
  ...
  def beEUnsat =
    beSome ^^ { ( f: HOLFormula ) => new Escargot( splitting = false, equality = true, propositional = true ).getRobinsonProof( f ) }
