...
val p: gapt.proofs.nd.ForallIntroRule =
...
val m: gapt.models.PropositionalModel =
R(p_1, h_1): o -> true
R(p_1, h_2): o -> false
R(p_1, h_3): o -> false
R(p_2, h_1): o -> false
...
val res0: Option[gapt.models.PropositionalModel] =
Some(a:o -> false
b:o -> false
c:o -> true)
...
val formula: gapt.expr.formula.fol.FOLFormula =
∀x ∀y ∀z x + y + z = x + (y + z) ∧ ∀x ∀y x + y = y + x →
  d + a + c + b = a + b + c + d

gapt> Escargot getResolutionProof formula
val res0: Option[gapt.proofs.resolution.ResolutionProof] =
...
[p38] c + (b + (a + d)) = d + (c + (a + b)) ⊢    (Paramod(p24, Suc(0), true, p37,...
...
∀y ∀z '+2'(S(z), y) = S('+2'(z, y))
⊢
∀n ∀m '-2'(n, '+2'(n, m)) = #c(Z: Nat)    (CutRule(p21, Suc(0), p170, Ant(3)))
[p170] ∀y0 #c(Z: Nat) != (S(y0:Nat): Nat),
∀x0 ('proj1-S'(S(x0)): Nat) = x0,
∀z ('-2'(S(z), #c(Z: Nat)): Nat) = S(z),
...
A second-order formula of the form $\exists X_1 \dots \exists X_n \varphi$, where the $X_i$ are second-order relation variables with arity $a_i$ and $\varphi$ is a first-order formula is called a \emph{formula equation}.
...
\begin{clilisting}
gapt> val formulaEquationSolution = solveFormulaEquation(hof"∃X X(a)")
val formulaEquationSolution: scala.util.Try[gapt.expr.subst.Substitution] = Success(Substitution(X:i>o -> λx_0 a = x_0))
...
Failure does not mean that the formula equation is not solvable, but only that the algorithm we use couldn't find a witness.
If the \texttt{Try} is successful the return value is a substitution such that applying the substitution to the first order part of the formula equation gives a valid formula.

Another version of this method is called \texttt{dls}.
This method solves the related problem of finding lambda expressions $\psi_i$ such that 
\begin{equation}
  \label{eq:second-order-elimination-with-witness}
  \exists X_1 \dots \exists X_n \varphi \leftrightarrow \varphi[X_1/\psi_1,\dots,X_n/\psi_n]
\end{equation}
is valid.
If successful, the return value is a tuple of a substitution and the first order part of the given formula equation (possibly simplified) such that applying the substitution to the first-order part fulfils that \eqref{eq:second-order-elimination-with-witness} is valid.
...
\begin{clilisting}
gapt> val solution = dls(hof"∃X X(a)")
val solution: scala.util.Try[(gapt.expr.subst.Substitution, gapt.expr.formula.Formula)] = Success((Substitution(X:i>o -> λx_0 a = x_0),X(a): o))
...
Both methods are based on the DLS algorithm in \cite{Doherty1997Computing}.
To get a more detailed description of the kinds of formulas for which the algorithm succeeds check the documentation of \texttt{solveFormulaEquation} and \texttt{dls}.
