...
% TODO(gabriel): uncomment when the global propositional subsumption bugfix lands
...
[p136] ∀y (plus(#c(Z: Nat), y:Nat): Nat) = y,
∀y0 ¬#c(Z: Nat) = S(y0:Nat),
∀x0 (p(S(x0)): Nat) = x0,
...
∀z ∀y plus(S(z), y) = S(plus(z, y))
⊢
∀n ∀m minus(n, plus(n, m)) = #c(Z: Nat)    (CutRule(p22, Suc(0), p135, Ant(4)))
[p135] ∀y (plus(#c(Z: Nat), y:Nat): Nat) = y,
∀y0 ¬#c(Z: Nat) = S(y0:Nat),
∀x0 (p(S(x0)): Nat) = x0,
∀z (minus(S(z), #c(Z: Nat)): Nat) = S(z),
∀m
