...
\begin{clilisting}
gapt> val derivation = scan(clauseSetPep, inferenceLimit=Some(10), attemptLimit=Some(10), oneSidedOnly=true).get
...
\end{clilisting}
The \texttt{inferenceLimit} parameter controls the number of derivation steps that are to be performed by SCAN.
...
One can set either or both of \texttt{inferenceLimit} and \texttt{attempLimit} to \texttt{None} to run SCAN without the respective limit.
...
\begin{clilisting}
gapt> val derivation = wscan(clauseSetPep, oneSidedOnly=true, inferenceLimit=Some(10), attemptLimit=Some(10), witnessLimit=Some(10)).get
