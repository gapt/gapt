...
val derivation: gapt.logic.hol.scan.Derivation = Derivation(ClauseSetPredicateEliminationProblem(Set(X:i>o),Set(X(b) ⊢ ,  ⊢ X(a))),List(ConstraintResolution(PointedClause(X(b) ⊢ ,Ant(0)),PointedClause( ⊢ X(a),Suc(0))), PurifiedClauseDeletion(PointedClause(X(b) ⊢ ,Ant(0))), ExtendendPurityDeletion(X:i>o,Positive)))
...
val res0: Set[gapt.proofs.HOLClause] = Set(b = a ⊢ )

\end{clilisting}
From the derivation we can also construct the witness substitution for the input PEP. 
\begin{clilisting}
gapt> val Some(witness) = wscan.witness(derivation, limit=Some(2)): @unchecked
val witness: gapt.expr.subst.Substitution = Substitution(X:i>o -> λu_0 b != u_0)

\end{clilisting}
During the construction of the witness some saturation processes similar to SCAN have to run. 
The parameter \texttt{limit} controls how many iterations during every one of these saturation processes we allow.
...
By the correctness of SCAN we know that the input formula $\exists X (X(a) \land \neg X(b))$ is equivalent to the conclusion of the SCAN derivation, namely the first-order formula $b \neq a$.
We can thus check if $b \neq a$ logically equivalent to substituting the computed witness for $X$ in the input formula by constructing the first-order WSOQE-condition and verifying that it is valid:
\begin{clilisting}
