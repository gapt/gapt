...
\end{lstlisting}
  ...
  \item If you like GAPT and want to use it as a library in your Scala project,
        ...
        \begin{lstlisting}
libraryDependencies += "at.logic.gapt" %% "gapt" % "2.20.0-SNAPSHOT"
\end{lstlisting}
...
val F: FOLFormula = ∀x (P(x, f(x)) → ∃y P(x, y))
...
val G: FOLFormula = ∀x (P(x, f(f(x))) → ∃y P(x, y))
...
val res0: Formula = ∀x ∃y ∀z (x(z): i) = y(y(z): i)
...
\begin{clilisting}
gapt> hof"∀x ∃y ∀z x(z) = y(y(z))"
val res1: Formula = ∀x ∃y ∀z (x(z): i) = y(y(z): i)
...
val f: HOLSequent = ((c_1 ∨ d_1) ∧ (c_2 ∨ d_2) ∧ (c_3 ∨ d_3) ∧ (c_4 ∨ d_4) → c_5) ∨
...
val res0: HOLSequent = P(a), a = b ⊢ P(b)
...
\begin{clilisting}
gapt> val S1 = fcl"B :- C"
val S1: FOLClause = B ⊢ C

gapt> val S2 = foa"A" +: S1
val S2: Sequent[FOLAtom] = A, B ⊢ C

gapt> val S3 = S2 :+ foa"D"
val S3: Sequent[FOLAtom] = A, B ⊢ C, D

gapt> foa"A" +: foa"B" +: Sequent() :+ foa"C" :+ foa"D"
val res1: Sequent[FOLAtom] = A, B ⊢ C, D
...
val p1: LogicalAxiom = [p1] A ⊢ A    (LogicalAxiom(A:o))

gapt> val p2 = LogicalAxiom(fof"B")
val p2: LogicalAxiom = [p1] B ⊢ B    (LogicalAxiom(B:o))
...
val p3: AndRightRule = [p3] A, B ⊢ A ∧ B    (AndRightRule(p1, Suc(0), p2, Suc(0)))
...
val p4: ImpRightRule = [p4] A ⊢ B → A ∧ B    (ImpRightRule(p3, Ant(1), Suc(0)))
[p3] A, B ⊢ A ∧ B    (AndRightRule(p1, Suc(0), p2, Suc(0)))
[p2] B ⊢ B    (LogicalAxiom(B:o))
[p1] A ⊢ A    (LogicalAxiom(A:o))

gapt> val p5 = ImpRightRule( p4, fof"A", fof"B -> A&B" )
val p5: ImpRightRule = [p5]  ⊢ A → B → A ∧ B    (ImpRightRule(p4, Ant(0), Suc(0)))
...
val p2: AndLeftRule = [p3] A ∧ B ⊢ A    (AndLeftRule(p2, Ant(1), Ant(0)))
...
val p: LKProof = [p25] ∀x ∀y (P(s(x), y) → P(x, s(y))), P(s(s(s(s(s(0))))), 0) ⊢ P(0, s(s(s(s(s(0))))))    (ContractionLeftRule(p24, Ant(0), Ant(1)))
...
[p21] P(0, s(s(s(s(s(0)))))) ⊢ P(0, s(s(s(s(s(0))))))    (LogicalAxiom(P(0, s(s(s(s(s(0)))))): o))
[p20] ∀x ∀y (P(s(x), y) → P(x, s(y))), P(s(s(s(s(s(0))))), 0) ⊢ P(s(0), s(s(s(s(0)))))    (ContractionLeftRule(p19, Ant(0), Ant(1)))
[p19] ∀x ∀y (P(s(x), y) → P(x, s(y))),
∀x ∀y (P(s(x), y) → P(x, s(y))),
... output truncated
...
val p1: nd.LogicalAxiom = [p1] P ⊢ P    (LogicalAxiom(P:o))

gapt> val p2 = nd.LogicalAxiom( fof"Q" )
val p2: nd.LogicalAxiom = [p1] Q ⊢ Q    (LogicalAxiom(Q:o))

gapt> val p3 = nd.LogicalAxiom( fof"P & Q -> R" )
val p3: nd.LogicalAxiom = [p1] P ∧ Q → R ⊢ P ∧ Q → R    (LogicalAxiom(P ∧ Q → R))
...
val p4: nd.AndIntroRule = [p3] P, Q ⊢ P ∧ Q    (AndIntroRule(p1, p2))
...
val p5: nd.ImpElimRule = [p5] P ∧ Q → R, P, Q ⊢ R    (ImpElimRule(p1, p4))
...
val p6: nd.ImpIntroRule = [p6] P ∧ Q → R, P ⊢ Q → R    (ImpIntroRule(p5, Ant(2)))
...
val subst: FOLSubstitution = Substitution(x -> f(a), y -> g(x))
...
val t: FOLTerm = #c(g: i>i>i)(f(a), g(x))

gapt> val f = subst(fof"P(x,y) & Q(x)")
val f: FOLFormula = P(f(a), g(x)) ∧ Q(f(a))

gapt> val seq = subst(fos"P(x) :- Q(y)")
val seq: Sequent[FOLFormula] = P(f(a)) ⊢ Q(g(x))

gapt> val p = AndRightRule(LogicalAxiom(fof"A(x)"), fof"A(x)", LogicalAxiom(fof"B(y)"), fof"B(y)")
val p: AndRightRule = [p3] A(x), B(y) ⊢ A(x) ∧ B(y)    (AndRightRule(p1, Suc(0), p2, Suc(0)))
[p2] B(y) ⊢ B(y)    (LogicalAxiom(B(y): o))
[p1] A(x) ⊢ A(x)    (LogicalAxiom(A(x): o))

gapt> val ps = subst(p)
val ps: LKProof = [p3] A(f(a)), B(g(x)) ⊢ A(f(a)) ∧ B(g(x))    (AndRightRule(p1, Suc(0), p2, Suc(0)))
...
val res0: FOLFormula = ∀x_0 P(x_0) ∧ Q(f(a))
...
val res1: FOLFormula = ∀x_0 P(x_0, g(x))
...
val substLeft: FOLSubstitution = Substitution(x -> g(y))

gapt> val substRight = FOLSubstitution(fov"y" -> fot"f(x)")
val substRight: FOLSubstitution = Substitution(y -> f(x))

gapt> substLeft.compose(substRight)
val res2: FOLSubstitution = Substitution(x -> g(y), y -> f(g(y)))

\end{clilisting}

Substitutions are composed like functions. This means in the composed substitution the right substitution is applied first and then the left substitution is applied to the result.

GAPT also provides convenience methods to apply an substitution to a given term in post-fix notation:
\begin{clilisting}
gapt> fot"g(x,y)".applySubstitution(FOLSubstitution(fov"x" -> fot"f(a)"))
val res3: FOLTerm = g(f(a), y)

\end{clilisting}

The \cli{substitute} method makes this even shorter:
\begin{clilisting}
gapt> fot"g(x,y)".substitute(fov"x" -> fot"f(a)")
val res4: FOLTerm = g(f(a), y)

\end{clilisting}

Note that giving the same variable twice in one \cli{substitute} call will ignore the first substitution pair:
\begin{clilisting}
gapt> fot"x".substitute(fov"x" -> fot"f(x)", fov"x" -> foc"b")
val res5: FOLTerm = b

\end{clilisting}

Use separate \cli{subsitute} calls for the composed result:
\begin{clilisting}
gapt> fot"x".substitute(fov"x" -> fot"f(x)").substitute(fov"x" -> foc"b")
val res6: FOLTerm = f(b)

\end{clilisting}

Higher-order substitutions (without the FOL prefix) function similarly, but the terms being substituted are now lambda expressions. Creating higher-order substitutions often involves annotating the types of the expressions explicitly:
\begin{clilisting}
gapt> val subst = Substitution(hov"X:i>o" -> le"^u P(u): o", hov"Y:i>i>i" -> le"^u^v u+v")
val subst: Substitution = Substitution(X:i>o -> λu P(u): o, Y:i>i>i -> λu λv u + v)

gapt> val p = subst(hof"X(Y(a,b))")
val p: Formula = (λu P(u))((λu λv u + v)(a, b): i): o

\end{clilisting}

As the previous example shows, higher-order substitutions usually introduce lambda terms that are not beta-normalized.
To get the beta-normalized version, one can use the postfix \cli{.betaNormalized}:
\begin{clilisting}
gapt> p.betaNormalized
val res7: Formula = P(a + b): o

\end{clilisting}

The \cli{substitute} method is also available for higher-order substitutions:

\begin{clilisting}
gapt> val f = hof"!u X(Y(u,z))"
val f: Formula = ∀u X(Y(u, z): i)

gapt> val fs = f.substitute(hov"X:i>o" -> le"^u P(u): o", hov"Y:i>i>i" -> le"^u^v u + v", fov"z" -> fot"g(u)")
val fs: Formula = ∀u_0 (λu P(u))((λu λv u + v)(u_0, g(u)): i)

gapt> val fsb = fs.betaNormalized
val fsb: Formula = ∀u_0 P(u_0 + g(u))
...
c: Q(a)
  ...
  at gapt.proofs.gaptic.Lemma$.finish(language.scala:42)
  at gapt.proofs.gaptic.Lemma$Helper.handleTacticBlock(language.scala:54)
  ... elided
\end{tacticsoutput}
...
val p: LKProof = [p25] ∀x ∀y (P(x, y) → Q(x, y)) ⊢ ∃x ∃y (¬Q(x, y) → ¬P(x, y))    (CutRule(p9, Suc(0), p24, Ant(0)))
...
[p18] Q(b, y) ⊢ ¬Q(b, y) → ¬P(b, y)    (ImpRightRule(p17, Ant(0), Suc(0)))
[p17] ¬Q(b, y), Q(b, y) ⊢ ¬P(b, y)    (WeakeningRightRule(p16, ¬P(b, y)))
[p16] ¬Q(b, y), Q(b, y) ⊢     (NegLeftRule(p15, Suc(0)))
[p15] Q(b, y) ⊢ Q(b, y)    (LogicalAxiom(Q(b, y): o))
[p14] ¬P(b, y) ⊢ ¬Q(b, y) → ¬P(b, y)    (NegLeftRule(p13, Suc(0)))
[p13]  ⊢ P(b, y), ¬Q(b, y) → ¬P(b, y)    (ImpRightRule(p12, Ant(0), Suc(1)))
[p12] ¬Q(b, y) ⊢ P(b, y), ¬P(b, y)    (WeakeningLeftRule(p11, ¬Q(b, y)))
[p11]  ⊢ P(b, y), ¬P(b, y)    (NegRightRule(p10, Ant(0)))
[p10] P(b, y) ⊢ P(b, y)    (LogicalAxiom(P(b, y): o))
[p9] ∀x ∀y (P(x, y) → Q(x, y)) ⊢ ∀x ∃y (¬P(x, y) ∨ Q(x, y))    (ForallRightRule(p8, Suc(0), x, x))
[p8] ∀x ∀y (P(x, y) → Q(x, y)) ⊢ ∃y (¬P(x, y) ∨ Q(x, y))    (ExistsRightRule(p7, Suc(0), ¬P(x, y) ∨ Q(x, y), a, y))
[p7] ∀x ∀y (P(x, y) → Q(x, y)) ⊢ ¬P(x, a) ∨ Q(x, a)    (ForallLeftRule(p6, Ant(0), ∀y (P(x, y) → Q(x, y)), x, x))
[p6] ∀y (P(x, y) → Q(x, y)) ⊢ ¬P(x, a) ∨ Q(x, a)    (ForallLeftRule(p5, Ant(0), P(x, y) → Q(x, y), a, y))
... output truncated
...
val q: LKProof = [p14] ∀x ∀y (P(x, y) → Q(x, y)) ⊢ ∃x ∃y (¬Q(x, y) → ¬P(x, y))    (ForallLeftRule(p13, Ant(0), ∀y (P(x, y) → Q(x, y)), b, x))
...
[p7] P(b, a) → Q(b, a) ⊢ ¬Q(b, a) → ¬P(b, a), ¬P(b, a)    (NegRightRule(p6, Ant(1)))
[p6] P(b, a) → Q(b, a), P(b, a) ⊢ ¬Q(b, a) → ¬P(b, a)    (ImpLeftRule(p1, Suc(0), p5, Ant(0)))
[p5] Q(b, a) ⊢ ¬Q(b, a) → ¬P(b, a)    (ImpRightRule(p4, Ant(0), Suc(0)))
[p4] ¬Q(b, a), Q(b, a) ⊢ ¬P(b, a)    (WeakeningRightRule(p3, ¬P(b, a)))
[p3] ¬Q(b, a), Q(b, a) ⊢     (NegLeftRule(p2, Suc(0)))
[p2] Q(b, a) ⊢ Q(b, a)    (LogicalAxiom(Q(b, a): o))
[p1] P(b, a) ⊢ P(b, a)    (LogicalAxiom(P(b, a): o))
...
val p: LKProof = [p118] ∀x ((x:nat) + (0:nat): nat) = x,
∀x ∀y x + s(y:nat) = s(x + y)
⊢
...
[p113] ∀x_0 ((x_0:nat) + (0:nat): nat) = x_0 ∧
  ∀x_0
  ∀y_1
  x_0 + s(y_1:nat) = s(x_0 + y_1)
⊢
∀x ∀y x + y = y + x    (ContractionLeftRule(p112, Ant(1), Ant(0)))
[p112] ∀x_1 ((x_1:nat) + (0:nat): nat) = x_1 ∧ ∀x_1 ∀y x_1 + s(y:nat) = s(x_1 + y),
∀x_0 x_0 + 0 = x_0 ∧ ∀x_0 ∀y_1 x_0 + s(y_1) = s(x_0 + y_1)
⊢
... output truncated

gapt> val q = inductionNormalForm(p)(using examples.theories.nat.ctx)
val q: LKProof = [p16] ∀x_1 ((x_1:nat) + (0:nat): nat) = x_1,
...
0 + 0 = 0
⊢
s(s(0)) + 0 = 0 + s(s(0))    (ForallLeftRule(p12, Ant(0), ((x_0:nat) + (0:nat): nat) = x_0, s(s(0:nat): nat), x_0:nat))
[p12] (s(s(0:nat): nat) + 0: nat) = s(s(0)),
... output truncated
...
val res0: ExpansionProof = ∀x_1 ((x_1:nat) + (0:nat): nat) = x_1
  +^{0} (0 + 0 = 0)-
...
val proofs: Set[ResolutionProof] = Set(
...
val p: ForallRightRule = [p2]  ⊢ ∀x x = x    (ForallRightRule(p1, Suc(0), x, x))
[p1]  ⊢ x = x    (ReflexivityAxiom(x))

gapt> skolemizeLK(p)
val res0: LKProof = [p2]  ⊢ ∀x x = x    (ForallSkRightRule(p1, Suc(0), ∀x x = x, s_0))
...
val p: LKProof = [p8] a → b, b → c ⊢ a ∧ d → c    (ImpRightRule(p7, Ant(0), Suc(0)))
...
val res0: LKProof = [p7] A → B ⊢ A ∧ B ∨ ¬A    (OrRightRule(p6, Suc(0), Suc(1)))
...
val res1: nd.NDProof = [p15] A → B ⊢ A ∧ B ∨ ¬A    (ExcludedMiddleRule(p2, Ant(0), p14, Ant(0)))
...
val expansion: ExpansionProof = ∀X (X → X)
  +^{∀x ∃y (¬P(x, y) ∨ Q(x, y))}
...
val res0: HOLSequent = ∀X (X → X), ∀x ∀y (P(x, y) → Q(x, y)) ⊢ ∃x ∃y (¬Q(x, y) → ¬P(x, y))

gapt> expansion.deep
val res1: HOLSequent = ¬P(x, a) ∨ Q(x, a) → ¬P(b, y) ∨ Q(b, y), P(x, a) → Q(x, a) ⊢ ¬Q(b, y) → ¬P(b, y)
...
val res3: ExpansionProof = ∀x ∀y (P(x, y) → Q(x, y)) +^{b, a} (P(b, a)+ → Q(b, a)-)
:-
...
val res4: LKProof = [p21] ∀x ∀y (P(x, y) → Q(x, y)) ⊢ ∃x ∃y (¬Q(x, y) → ¬P(x, y))    (ExistsRightRule(p20, Suc(0), ∃y (¬Q(x, y) → ¬P(x, y)), b, x))
...
[p14] ¬Q(b, y), ¬P(b, y) ∨ Q(b, y), P(b, y) ⊢     (NegLeftRule(p13, Suc(0)))
[p13] ¬P(b, y) ∨ Q(b, y), P(b, y) ⊢ Q(b, y)    (OrLeftRule(p11, Ant(0), p12, Ant(0)))
[p12] Q(b, y) ⊢ Q(b, y)    (LogicalAxiom(Q(b, y): o))
[p11] ¬P(b, y), P(b, y) ⊢     (NegLeftRule(p10, Suc(0)))
[p10] P(b, y) ⊢ P(b, y)    (LogicalAxiom(P(b, y): o))
[p9] ∀x ∀y (P(x, y) → Q(x, y)) ⊢ ∀x ∃y (¬P(x, y) ∨ Q(x, y))    (ForallRightRule(p8, Suc(0), x, x))
[p8] ∀x ∀y (P(x, y) → Q(x, y)) ⊢ ∃y (¬P(x, y) ∨ Q(x, y))    (ForallLeftRule(p7, Ant(0), ∀y (P(x, y) → Q(x, y)), x, x))
[p7] ∀y (P(x, y) → Q(x, y)) ⊢ ∃y (¬P(x, y) ∨ Q(x, y))    (ForallLeftRule(p6, Ant(0), P(x, y) → Q(x, y), a, y))
[p6] P(x, a) → Q(x, a) ⊢ ∃y (¬P(x, y) ∨ Q(x, y))    (ExistsRightRule(p5, Suc(0), ¬P(x, y) ∨ Q(x, y), a, y))
[p5] P(x, a) → Q(x, a) ⊢ ¬P(x, a) ∨ Q(x, a)    (OrRightRule(p4, Suc(1), Suc(0)))
[p4] P(x, a) → Q(x, a) ⊢ Q(x, a), ¬P(x, a)    (NegRightRule(p3, Ant(1)))
[p3] P(x, a) → Q(x, a), P(x, a) ⊢ Q(x, a)    (ImpLeftRule(p1, Suc(0), p2, Ant(0)))
[p2] Q(x, a) ⊢ Q(x, a)    (LogicalAxiom(Q(x, a): o))
... output truncated
...
\section{Elimination of set equality reasoning}

Equational reasoning on sets is realised in gapt by using the $(\mt{=:l})$ and $(\mt{=:r})$
 rules on equations of the type set.
In addition, it is often useful to include the axiom of extensionality for sets:
\[
\forall A \forall B\, ( A = B  \leftrightarrow \forall x (A(x) \leftrightarrow B(x)) )
\]
The transformation \cli{eliminateSetEquality} takes an LK proof as input and eliminates equality
 reasoning on sets including the set extensionality axiom.
This is done by replacing set equalities by first-order equivalences throughout the proof
 including the replacement of set equality inferences by first-order cuts.
See \cli{examples/ringinv/ringinv.scala} for an example for how to use this transformation.
...
val p: nd.ForallIntroRule = [p4]  ⊢ ∀x ∃y (y:nat) = ((s(0:nat): nat) + (x:nat): nat)    (ForallIntroRule(p3, x:nat, x:nat))
...
\begin{clilisting}
gapt> val mr = nd.MRealizability.mrealize(p)(using examples.successor.ctx)
val mr: (Map[SequentIndex, Var], Expr) = (Map(), λx s(x:nat): nat)

\end{clilisting}

Examples of proofs from which more interesting realizers are extracted by the algorithm can be found in the file \cli{examples/m-realizabilityExamples.scala}.
...
val sequent: HOLSequent = A, B ⊢ P(x)
...
val ax1: LogicalAxiom = [p1] A ⊢ A    (LogicalAxiom(A:o))

gapt> val ax2 = LogicalAxiom(hof"B")
val ax2: LogicalAxiom = [p1] B ⊢ B    (LogicalAxiom(B:o))

gapt> val p1 = AndRightRule(ax1, hof"A", ax2, hof"B")
val p1: AndRightRule = [p3] A, B ⊢ A ∧ B    (AndRightRule(p1, Suc(0), p2, Suc(0)))
[p2] B ⊢ B    (LogicalAxiom(B:o))
[p1] A ⊢ A    (LogicalAxiom(A:o))

gapt> val p2 = AndRightRule(p1, hof"A∧B", ax1, hof"A")
val p2: AndRightRule = [p4] A, B, A ⊢ A ∧ B ∧ A    (AndRightRule(p3, Suc(0), p1, Suc(0)))
[p3] A, B ⊢ A ∧ B    (AndRightRule(p1, Suc(0), p2, Suc(0)))
[p2] B ⊢ B    (LogicalAxiom(B:o))
[p1] A ⊢ A    (LogicalAxiom(A:o))

gapt> val p3: LKProof = AndRightRule(p2, hof"A∧B∧A", ax2, hof"B")
val p3: LKProof = [p5] A, B, A, B ⊢ A ∧ B ∧ A ∧ B    (AndRightRule(p4, Suc(0), p2, Suc(0)))
...
val sequent: HOLSequent = p(0), ∀x (p(x) → p(s(x))) ⊢ p(s(s(0)))

gapt> TptpFOLExporter(sequent)
val res0: TptpFile = fof(ant_0, axiom, p('0')).
...
val tptp: TptpFile = fof(a, axiom, i(sr2)).
fof(b, axiom, ~ i(two)).
...
gapt> EProver getLKProof sequent
  val res4: Option[gapt.proofs.lk.LKProof] =
  Some([p11] ∀x (p(x) → p(s(x))), p(0) ⊢ p(s(s(0)))    (ContractionLeftRule(p10, Ant(2), Ant(1)))
  [p10] p(0), ∀x (p(x) → p(s(x))), ∀x (p(x) → p(s(x))) ⊢ p(s(s(0)))    (CutRule(p5, Suc(0), p9, Ant(1)))
  [p9] ∀x (p(x) → p(s(x))), p(s(0)) ⊢ p(s(s(0)))    (CutRule(p8, Suc(0), p6, Ant(0)))
  [p8] ∀x (p(x) → p(s(x))), p(s(0)) ⊢ p(s(s(0)))    (ForallLeftRule(p7, Ant(0), p(x) → p(s(x)), s(0), x))
...
gapt> Prover9 getExpansionProof hof"?x!y p x y -> !y?x p x y"
  val res5: Option[gapt.proofs.expansion.ExpansionProof] =
  Some(
  :-
...
gapt> LeanCoP getExpansionProof sequent
  val res6: Option[gapt.proofs.expansion.ExpansionProof] =
  Some(∀x (p(x) → p(s(x))) +^{0} (p(0)+ → p(s(0))-) +^{s(0)} (p(s(0))+ → p(s(s(0)))-),
  p(0)-
  :-
  p(s(s(0)))+)
...
val reduction:
  Reduction[HOLSequent, Sequent[Formula], ExpansionProof, ExpansionProof] = PredicateReductionET |> ErasureReductionET

gapt> val problem = hos"!(x:a)!y x=y, !x f(f(x:b))=x :- ?y c=f(y)"
val problem: HOLSequent = ∀x ∀y (x:a) = (y:a), ∀x f(f(x:b): b) = x ⊢ ∃y (c:b) = f(y)

gapt> val (firstOrderProblem, back) = reduction.forward(problem)
val firstOrderProblem: Sequent[Formula] = ∀x0 (P_is_b(x0) → P_is_b(f_f(x0))),
⊤ → P_is_b(f_c),
∃x P_is_b(x),
∃x P_is_a(x),
...
val back: ExpansionProof => ExpansionProof = <function>

gapt> Escargot.getExpansionProof(firstOrderProblem).map(back)
val res11: Option[ExpansionProof] = Some(
  ∀x f(f(x:b): b) = x +^{c} (f(f(c)) = c)-
:-
...
val f: Formula = (a = b ∨ a = c) ∧ P(c) ∧ P(b) → P(a)
...
(check-sat)
"""
val typeRenaming: Map[TBase, TBase] = Map(o -> Bool, i -> t_i)
val constantRenaming: Map[Const, Const] = Map(
  a -> f_a:t_i,
  b -> f_b:t_i,
  c -> f_c:t_i,
  P:i>o -> f_P:t_i>Bool
...
Furthermore,
\begin{clilisting}
gapt> val Some(m) = Sat4j.solve(-PigeonHolePrinciple(3, 3)): @unchecked
val m: models.PropositionalModel = R(p_1, h_1): o -> true
R(p_1, h_2): o -> false
R(p_1, h_3): o -> false
R(p_2, h_1): o -> false
R(p_2, h_2): o -> true
...
val p1: FOLAtom = R(p_1, h_1): o

gapt> val p2 = PigeonHolePrinciple.inHole(2, 1)
val p2: FOLAtom = R(p_2, h_1): o
...
val cnf: Set[Sequent[Atom]] = Set(
   ⊢ R(p_3, h_3), R(p_3, h_1), R(p_3, h_2),
  R(p_1, h_2), R(p_3, h_2) ⊢ ,
  R(p_1, h_2), R(p_2, h_2) ⊢ ,
  R(p_1, h_3), R(p_2, h_3) ⊢ ,
  R(p_2, h_1), R(p_3, h_1) ⊢ ,
   ⊢ R(p_1, h_3), R(p_1, h_1), R(p_1, h_2),
  R(p_1, h_1), R(p_2, h_1) ⊢ ,
  R(p_1, h_1), R(p_3, h_1) ⊢ ,
   ⊢ R(p_2, h_3), R(p_2, h_1), R(p_2, h_2),
  R(p_1, h_3), R(p_3, h_3) ⊢ ,
  R(p_2, h_3), R(p_3, h_3) ⊢ ,
  R(p_2, h_2), R(p_3, h_2) ⊢ 
)
...
val encoding: DIMACSEncoding = DIMACSEncoding()
...
val res13: Option[RupProof] = Some(
  c input 12 11 8 0
c input -1 -2 0
c input -4 -8 0
c input 7 9 1 0
...
val res0: Option[models.PropositionalModel] = Some(
  a:o -> false
b:o -> false
c:o -> true
)
...
val formula: FOLFormula = ∀x ∀y ∀z x + y + z = x + (y + z) ∧ ∀x ∀y x + y = y + x →
  d + a + c + b = a + b + c + d

gapt> Escargot.getResolutionProof(formula)
val res0: Option[ResolutionProof] = Some(
  [p44]  ⊢    (Resolution(p1, Suc(0), p43, Ant(0)))
...
[p38] c + (b + (a + d)) = d + (c + (a + b)) ⊢    (Paramod(p24, Suc(0), true, p37, Ant(0), λx x = d + (c + (a + b))))
[p37] b + (c + (a + d)) = d + (c + (a + b)) ⊢    (Flip(p36, Ant(0)))
[p36] d + (c + (a + b)) = b + (c + (a + d)) ⊢    (Paramod(p25, Suc(0), true, p35, Ant(0), λx x = b + (c + (a + d))))
[p35] c + (a + b) + d = b + (c + (a + d)) ⊢    (Paramod(p26, Suc(0), true, p34, Ant(0), λx c + (a + b) + d = b + (c + x)))
[p34] c + (a + b) + d = b + (c + (d + a)) ⊢    (Paramod(p27, Suc(0), true, p33, Ant(0), λx c + (a + b) + d = x))
[p33] c + (a + b) + d = c + (d + a) + b ⊢    (Paramod(p28, Suc(0), true, p32, Ant(0), λx x + d = c + (d + a) + b))
[p32] a + b + c + d = c + (d + a) + b ⊢    (Paramod(p29, Suc(0), true, p31, Ant(0), λx a + b + c + d = x + b))
[p31] a + b + c + d = d + a + c + b ⊢    (Flip(p30, Ant(0)))
[p30] d + a + c + b = a + b + c + d ⊢    (ImpL2(p2, Ant(0)))
[p29]  ⊢ d + a + c = c + (d + a)   (Subst(p12, Substitution(x -> d + a, y -> c)))
[p28]  ⊢ a + b + c = c + (a + b)   (Subst(p12, Substitution(x -> a + b, y -> c)))
[p27]  ⊢ c + (d + a) + b = b + (c + (d + a))   (Subst(p12, Substitution(x -> c + (d + a), y -> b)))
[p26]  ⊢ d + a = a + d   (Subst(p12, Substitution(x -> d, y -> a)))
... output truncated
...
val formula: Formula = P(cons(0:nat, cons(s(0): nat, nil:list): list)): o

gapt> val axiom = hof"P(nil) & !x!y!z (P(x) -> P(cons(y: nat, cons(z, x)): list))"
val axiom: Formula = P(nil:list) ∧ ∀x ∀y ∀z (P(x) → P(cons(y:nat, cons(z:nat, x): list)))

gapt> val problem = hof"$axiom -> $formula"
val problem: Formula = P(nil:list) ∧ ∀x ∀y ∀z (P(x) → P(cons(y:nat, cons(z:nat, x): list))) →
  P(cons(0, cons(s(0), nil)))

gapt> Escargot.getExpansionProof(problem)
val res1: Option[ExpansionProof] = Some(
...
val problem: TipProblem = ∀x0 'proj1-S'(S(x0)) = x0,
∀y '-2'(Z, y) = Z,
∀z '-2'(S(z), Z) = S(z),
...
val proof: LKProof = [p168] ∀y0 #c(Z: Nat) != (S(y0:Nat): Nat),
...
val res0: LKProof = [p5]  ⊢ a → b → a ∧ b    (ImpRightRule(p4, Ant(0), Suc(0)))
[p4] a ⊢ b → a ∧ b    (ImpRightRule(p3, Ant(1), Suc(0)))
[p3] a, b ⊢ a ∧ b    (AndRightRule(p1, Suc(0), p2, Suc(0)))
[p2] b ⊢ b    (LogicalAxiom(b:o))
[p1] a ⊢ a    (LogicalAxiom(a:o))
...
val res1: LKProof = [p9]  ⊢ a = b ∧ f(a) = b → a = f(f(b))    (ImpRightRule(p8, Ant(0), Suc(0)))
...
val res0: Option[LKProof] = Some(
  [p8]  ⊢ ¬ ¬ ∀x P(x) → ∀x ¬ ¬P(x)    (ImpRightRule(p7, Ant(0), Suc(0)))
...
gapt> val acnf = CERES(Pi2Pigeonhole.proof)
  val acnf: gapt.proofs.lk.LKProof =
...
gapt> val et = LKToExpansionProof(acnf)
  val et: gapt.proofs.expansion.ExpansionProof =
  ∀x ∀y (x <= M(x, y) ∧ y <= M(x, y))
...
val p: LKProof = [p36] ∀x (P(x) → P(s(x))), P(0) ⊢ P(s(s(s(s(s(s(s(s(s(0))))))))))    (ContractionLeftRule(p35, Ant(0), Ant(1)))
...
[p31] ∀x (P(x) → P(s(x))), ∀x (P(x) → P(s(x))), P(0) ⊢ P(s(s(s(s(s(s(s(s(0)))))))))    (ForallLeftRule(p30, Ant(0), P(x) → P(s(x)), s(s(s(s(s(s(s(0))))))), x))
[p30] P(s(s(s(s(s(s(s(0)))))))) → P(s(s(s(s(s(s(s(s(0))))))))),
∀x (P(x) → P(s(x))),
P(0)
⊢
P(s(s(s(s(s(s(s(s(0)))))))))    (ImpLeftRule(p28, Suc(0), p29, Ant(0)))
[p29] P(s(s(s(s(s(s(s(s(0))))))))) ⊢ P(s(s(s(s(s(s(s(s(0)))))))))    (LogicalAxiom(P(s(s(s(s(s(s(s(s(0))))))))): o))
[p28] ∀x (P(x) → P(s(x))), P(0) ⊢ P(s(s(s(s(s(s(s(0))))))))    (ContractionLeftRule(p27, Ant(0), Ant(1)))
[p27] ∀x (P(x) → P(s(x))), ∀x (P(x) → P(s(x))), P(0) ⊢ P(s(s(s(s(s(s(s(0))))))))    (ForallLeftRule(p26, Ant(0), P(x) → P(s(x)), s(s(s(s(s(s(0)))))), x))
[p26] P(s(s(s(s(s(s(0))))))) → P(s(s(s(s(s(s(s(0)))))))),
∀x (P(x) → P(s(x))),
... output truncated
...
val q: Option[LKProof] = Some(
  [p27] ∀x (P(x) → P(s(x))), P(0) ⊢ P(s(s(s(s(s(s(s(s(s(0))))))))))    (CutRule(p14, Suc(0), p26, Ant(0)))
...
P(0)
⊢
P(s(s(s(s(s(s(s(s(s(0))))))))))    (ForallLeftRule(p22, Ant(2), P(x1) → P(s(s(s(x1)))), 0, x1))
[p22] ∀x1 (P(x1) → P(s(s(s(x1))))),
... output truncated
...
val inputPep: ClauseSetPredicateEliminationProblem = ClauseSetPredicateEliminationProblem(
  varsToEliminate = List(X:i>o),
  firstOrderClauses = Set( ⊢ X(b), X(c) ⊢ ,  ⊢ X(a))
)
...
val wit: Substitution = Substitution(X:i>o -> λu_0 c != u_0)
...
val clauseSetPep: ClauseSetPredicateEliminationProblem = ClauseSetPredicateEliminationProblem(
  varsToEliminate = List(X:i>o),
  firstOrderClauses = Set( ⊢ X(s_2), X(s_4) ⊢ ,  ⊢ X(s_0))
)
...
\begin{clilisting}
gapt> val derivation = scan(clauseSetPep, stepLimit=Some(10), attemptLimit=Some(10), oneSidedOnly=true).get
val derivation: scan.Derivation = Derivation(
  from = ClauseSetPredicateEliminationProblem(
    varsToEliminate = List(X:i>o),
    firstOrderClauses = Set( ⊢ X(s_2), X(s_4) ⊢ ,  ⊢ X(s_0))
  ),
  derivationSteps = List(
    ConstraintResolution(
      left = PointedClause(clause = X(s_4) ⊢ , index = Ant(0)),
      right = PointedClause(clause =  ⊢ X(s_2), index = Suc(0))
    ),
    ConstraintResolution(
      left = PointedClause(clause = X(s_4) ⊢ , index = Ant(0)),
      right = PointedClause(clause =  ⊢ X(s_0), index = Suc(0))
    ),
    PurifiedClauseDeletion(PointedClause(clause = X(s_4) ⊢ , index = Ant(0))),
    ExtendendPurityDeletion(variable = X:i>o, polarity = Positive)
  )
)

\end{clilisting}
The \texttt{stepLimit} parameter controls the number of derivation steps that are to be performed by SCAN.
...
One can set either or both of \texttt{stepLimit} and \texttt{attempLimit} to \texttt{None} to run SCAN without the respective limit.
...
val res0: Set[HOLClause] = Set(s_4 = s_0 ⊢ , s_4 = s_2 ⊢ )
...
val wit: Substitution = Substitution(X:i>o -> λu_0 s_4 != u_0)
...
\begin{clilisting}
gapt> val derivation = wscan(clauseSetPep, oneSidedOnly=true, stepLimit=Some(10), attemptLimit=Some(10), witnessLimit=Some(10)).get
val derivation: Substitution = Substitution(X:i>o -> λu_0 s_4 != u_0)
...
val witnesses: Seq[Substitution] = List(
  Substitution(X:i>o -> λu_0 s_4 != u_0),
  Substitution(X:i>o -> λu_0 s_4 != u_0 ∨ s_2 = u_0),
  Substitution(X:i>o -> λu_0 s_0 = u_0 ∨ s_2 = u_0),
  Substitution(X:i>o -> λu_0 s_2 = u_0 ∨ s_0 = u_0),
  Substitution(X:i>o -> λu_0 s_4 != u_0 ∨ s_0 = u_0)
)
...
\begin{clilisting}
gapt> val nonEquivalent = wscan.mutuallyNonEquivalentWitnesses(clauseSetPep).toSeq
val nonEquivalent: Seq[Substitution] = List(
  Substitution(X:i>o -> λu_0 s_4 != u_0),
  Substitution(X:i>o -> λu_0 s_0 = u_0 ∨ s_2 = u_0)
)
...
with respect to the background theory induced by the first-order conclusion of the SCAN derivation.
...
val H: PredicateEliminationProblem = PredicateEliminationProblem(
  varsToEliminate = List(X:i>o, Y:i>o),
  firstOrderPart = X(a) ∧ ¬Y(a)
)
...
val solution: Substitution = Substitution(X:i>o -> λx_0 a = x_0, Y:i>o -> λy_0 ⊥)
...
val F: PredicateEliminationProblem = PredicateEliminationProblem(
  varsToEliminate = List(X:i>o),
  firstOrderPart = X(a) ∧ X(b)
)
...
val s: Substitution = Substitution(X:i>o -> λx_0 a = x_0 ∨ b = x_0)

\end{clilisting}

On success a substitution is returned, which when applied to the input formula yields a valid formula.
\begin{clilisting}
gapt> Escargot.isValid( BetaReduction.betaNormalize( s(F.firstOrderPart) ) )
...
The function \texttt{solveFormulaEquation} proceeds by solving WSOQE using WDLS and checking whether the resulting formula is valid using \texttt{Escargot.isValid}.
