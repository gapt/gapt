...
By default, \texttt{skolemizeInference} will keep eigenvariable inferences above cuts.
...
res43: at.logic.gapt.expr.Atom = R(p_3, h_1): o
...
[p136] ∀z ('-2'(S(z:Nat): Nat, #c(Z: Nat)): Nat) = S(z),
...
∀z ∀y '+2'(S(z), y) = S('+2'(z, y))
⊢
∀n ∀m '-2'(n, '+2'(n, m)) = #c(Z: Nat)    (CutRule(p22, Suc(0), p135, Ant(4)))
[p135] ∀z ('-2'(S(z:Nat): Nat, #c(Z: Nat)): Nat) = S(z),
∀x0 ('proj1-S'(S(x0)): Nat) = x0,
∀y0 ¬#c(Z: Nat) = S(y0),
∀y ('+2'(#c(Z: Nat), y:Nat): Nat) = y,
∀m
  ((⊤ ⊃ '-2'(#c(Z: Nat), '+2'(#c(Z: Nat), m)) = #c(Z: Nat)) ∧
      ∀n_0
        ('-2'(n_0, '+2'(n_0, m)) = #c(Z: Nat) ⊃
          '-2'(S(n_0), '+2'(S(n_0), m)) = #c(Z: Nat)) ⊃
    ∀n '-2'(n, '+2'(n, m)) = #c(Z: Nat)),
∀y '-2'(#c(Z: Nat), y) = #c(Z:...
