...
F: gapt.expr.formula.FOLFormula = ∀x (P(x, f(x)) → ∃y P(x, y))
...
t: gapt.expr.formula.FOLTerm = f(f(x))

gapt> val G = fof"!x (P(x,$t) -> ?y P(x,y))"
G: gapt.expr.formula.FOLFormula = ∀x (P(x, f(f(x))) → ∃y P(x, y))
...
fx: gapt.expr.formula.FOLTerm = f(x)

gapt> val Pfx = FOLAtom("P", fx)
Pfx: gapt.expr.formula.FOLAtom = P(f(x)): o
...
A: gapt.expr.formula.FOLAtom = A:o

gapt> val B = FOLAtom("B")
B: gapt.expr.formula.FOLAtom = B:o

gapt> val C = FOLAtom("C")
C: gapt.expr.formula.FOLAtom = C:o

gapt> (A & B) --> C
res2: gapt.expr.formula.FOLFormula = A ∧ B → C
...
S3: gapt.proofs.Sequent[gapt.expr.formula.FOLAtom] = A, B ⊢ C, D
...
S2: gapt.proofs.Sequent[gapt.expr.formula.FOLAtom] = A, B ⊢ C

gapt> val S3 = S2 :+ foa"D"
S3: gapt.proofs.Sequent[gapt.expr.formula.FOLAtom] = A, B ⊢ C, D

gapt> foa"A" +: foa"B" +: Sequent() :+ foa"C" :+ foa"D"
res1: gapt.proofs.Sequent[gapt.expr.formula.FOLAtom] = A, B ⊢ C, D
...
gapt> val d = S(j)
d: gapt.expr.formula.FOLAtom = D:o
...
c: Q(a)
  ...
  at gapt.proofs.gaptic.Lemma$.finish(language.scala:32)
  at gapt.proofs.gaptic.Lemma$.finish(language.scala:36)
  at gapt.proofs.gaptic.Lemma$Helper.handleTacticBlock(language.scala:48)
  ... elided
\end{tacticsoutput}
...
p1: gapt.expr.formula.FOLAtom = R(p_1, h_1): o

gapt> val p2 = PigeonHolePrinciple.inHole(2, 1)
p2: gapt.expr.formula.FOLAtom = R(p_2, h_1): o
...
formula: gapt.expr.formula.FOLFormula =
