...
\begin{clilisting}
gapt> val inputPep = pep"?X (X(a) & -X(b))"
val inputPep: gapt.logic.hol.PredicateEliminationProblem = PredicateEliminationProblem(Set(X:i>o),X(a) ∧ ¬X(b))
...
val clauseSetPep: gapt.logic.hol.ClauseSetPredicateEliminationProblem = ClauseSetPredicateEliminationProblem(Set(X:i>o),Set(X(b) ⊢ ,  ⊢ X(a)))
...
\begin{clilisting}
gapt> val Success(derivation) = scan(clauseSetPep, limit=Some(100), oneSidedOnly=true): @unchecked
-- Error: ----------------------------------------------------------------------
1 |val Success(derivation) = scan(clauseSetPep, limit=Some(100), oneSidedOnly=true): @unchecked
  |                                             ^^^^^^^^^^^^^^^
  |method apply in object scan: (input: gapt.logic.hol.ClauseSetPredicateEliminationProblem,
  |  derivationLimit: Option[Int], witnessLimit: Int):
  |  Iterator[
  |    Either[gapt.logic.hol.scan.scan.Derivation, (Set[gapt.proofs.HOLClause],
  |      Option[gapt.expr.subst.Substitution], gapt.logic.hol.scan.scan.Derivation)
  |      ]
  |  ] does not have a parameter limit
-- Error: ----------------------------------------------------------------------
1 |val Success(derivation) = scan(clauseSetPep, limit=Some(100), oneSidedOnly=true): @unchecked
  |                                                              ^^^^^^^^^^^^^^^^^
  |method apply in object scan: (input: gapt.logic.hol.ClauseSetPredicateEliminationProblem,
  ...
  |      Option[gapt.expr.subst.Substitution], gapt.logic.hol.scan.scan.Derivation)
  |      ]
  |  ] does not have a parameter oneSidedOnly
...
\end{clilisting}
The \texttt{limit} parameter controls the number of derivation steps that are to be performed by SCAN.
If SCAN does not saturate the input clause set within this limit, the method returns with an Error.
One can set \texttt{limit=None} to allow SCAN to run without a limit.
...
\begin{clilisting}
gapt> val Success(witness) = wscan.witness(derivation, limit=Some(2)): @unchecked
-- [E006] Not Found Error: -----------------------------------------------------
1 |val Success(witness) = wscan.witness(derivation, limit=Some(2)): @unchecked
  |                       ^^^^^
  |                       Not found: wscan
  |
  | longer explanation available when compiling with `-explain`
...
We can now use this witness to construct the corresponding WSOQE condition
\begin{clilisting}
gapt> val wsoqeCondition = wsoqe.condition(clauseSetPep.toFormula, witness)
-- [E006] Not Found Error: -----------------------------------------------------
1 |val wsoqeCondition = wsoqe.condition(clauseSetPep.toFormula, witness)
  |                     ^^^^^
  |                     Not found: wsoqe
  |
  | longer explanation available when compiling with `-explain`
...
\end{clilisting}
and verify that the found witness satisfies it
\begin{clilisting}
gapt> Escargot.isValid(wsoqeCondition)
-- [E006] Not Found Error: -----------------------------------------------------
1 |Escargot.isValid(wsoqeCondition)
  |                 ^^^^^^^^^^^^^^
  |                 Not found: wsoqeCondition
  |
  | longer explanation available when compiling with `-explain`
1 error found
...
% TODO: add parameter for back-tracking
% TODO: add method that does all these steps in one go for convenience
% TODO: mention computing multiple witnesses
% TODO: mention where further examples can be found
