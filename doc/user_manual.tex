...
\begin{clilisting}
gapt> import gapt.formats.json._
import gapt.formats.json._

gapt> val s = JSONExporter(hos"A, B :- P(x)")
s: String =
{
  "antecedent" : [
    "A:o",
...
\begin{clilisting}
gapt> import gapt.formats.InputFile
import gapt.formats.InputFile

gapt> val sequent = JSONImporter[HOLSequent](InputFile.fromString(s))
...
res23: gapt.formats.tptp.TptpFile =
fof(ant_0, axiom, p('0')).
...
res26: Option[gapt.proofs.resolution.ResolutionProof] =
...
res29: Option[gapt.proofs.lk.LKProof] =
Some([p11] ∀x (p(x) → p(s(x))), p(0) ⊢ p(s(s(0)))    (ContractionLeftRule(p10, Ant(2), Ant(1)))
[p10] p(0), ∀x (p(x) → p(s(x))), ∀x (p(x) → p(s(x))) ⊢ p(s(s(0)))    (CutRule(p5, Suc(0), p9, Ant(1)))
[p9] ∀x (p(x) → p(s(x))), p(s(0)) ⊢ p(s(s(0)))    (CutRule(p8, Suc(0), p6, Ant(0)))
[p8] ∀x (p(x) → p(s(x))), p(s(0)) ⊢ p(s(s(0)))    (ForallLeftRule(p7, Ant(0), p(x) → p(s(x)), s(0), x))
...
res30: Option[gapt.proofs.expansion.ExpansionProof] =
Some(
:-
...
res31: Option[gapt.proofs.expansion.ExpansionProof] =
Some(∀x (p(x) → p(s(x))) +^{0} (p(0)+ → p(s(0))-) +^{s(0)} (p(s(0))+ → p(s(s(0)))-),
p(0)-
:-
p(s(s(0)))+)
...
res47: Option[gapt.proofs.rup.RupProof] =
Some(c input -10 -6 0
c input -11 -3 0
c input -1 -3 0
c input -11 -2 0
...
\begin{clilisting}
gapt> val problem = TipSmtImporter.parse(FilePath("examples/tip/isaplanner/prop_06.smt2"))
