...
%commented out because might not hold after a merge
% --- formally, if $\Delta_G(t_1,\dots,t_n) = (u,S)$, then the leftmost instance of $\alpha$ in $u$ is $\alpha_1$ and $\alpha_{j_1}$ is the nearest left sibling of $\alpha_{j_2}$ iff $j_1+1=j_2$.\\
...
$$
  \Delta_G^K(t_1,\dots,t_n) = \left\{
    ...
    \begin{array}{l l}
      ...
      \\
      & ((\textbf{RANDOM} \mt{ = true and}\\
      & \ \ \pi_2(\texttt{nub}(f(u_1,\dots,u_m), (\overline{s_1},\dots,\overline{s_q}))) \leq k) \mt{ or}\\
      & \ m = 1)\\
      \\
      \\
      \mt{where } & (\overline{s_1},\dots,\overline{s_q}) = \bigsqcup\limits_{1 \leq j \leq m} \pi_2(\Delta_G(t_j^1,\dots,t_j^n)) \mt{ and}\\
