...
case class DumpData( path: String, file_list: String ) {
  ...
  val parsed = files.map( x => FilePath( path + x ) ).par.flatMap( x =>
    try {
      val r = ( x, tptpInferTypes( TptpParser.load( x ) ).inputs.collect {
  ...
  val seqs = parsed.toSet.par.flatMap( ( x: ( FilePath, Set[Set[Formula]] ) ) => x._2 )
...
trait Simplifier[T] {
  ...
  def simplifyC( inset: Set[Expr], cache: T ): Set[Expr] = {
  ...
  def simplify( e: Expr, cache: Caches.C1 ): Expr = {
    ...
    e match {
      ...
      case NonLogicalConstant( name, ty, _ ) =>
        ...
        val nc = Const( ng.fresh( base ), simplifyT( ty ) )
        cmap += ( ( ty, nc ) )
        nc
...
object UninterpretedSimplifier extends Simplifier[Caches.C2] {
  ...
  val posnum = Const( "posN", TInt )
  val negnum = Const( "negN", TInt )
  val num = Const( "num", TInt )
    ...
    e match {
      ...
      case NonLogicalConstant( name, TInt, _ ) if tptpInferTypes.isNumeral( name ) =>
        ...
        val nc = name match {
          case "0"                                    => Zero
          case "1"                                    => One
          case num if num.nonEmpty && num( 0 ) == '-' => negnum
          case _                                      => posnum
        }*/
        num
...
case class Dump[T]( data: DumpData, simp: Simplifier[T] ) {
  ...
  lazy val pairmap = {
    ...
    for ( cl <- data.seqs ) {
      val ecl = cl.asInstanceOf[Set[Expr]]
      val simple = simp.simplifyC( ecl, cache )
