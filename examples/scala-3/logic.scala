    ...
    def apply(proofName: Expr, proofWithLinks: LKProof): DelayedProofResult = {
      ...
      val proofWithoutLinks0 = new LKVisitor[Unit] {
        override def visitProofLink(proof: ProofLink, otherArg: Unit): (LKProof, SequentConnector) =
          if (proof.conclusion.sizes != (0, 1)) super.visitProofLink(proof, otherArg)
          else {
            ...
            newProof -> SequentConnector(
              lowerSequent = newProof.conclusion,
              upperSequent = proof.conclusion,
...
class Theory(imports: Theory*) extends Theory0(imports.toList) {
  ...
  private object auxLemma {
    inline def apply[U](name: String, formula: Formula)(inline tacticsBlock: => Tactic[U]): Expr = new auxLemma(name, formula).applyTactics(tacticsBlock)
  }
  ...
  protected def indTy(ty0: Ty, ctrs: Const*): Unit = {
    ...
    for (ctr <- ctrs) {
      ...
      auxLemma(
        discrLemName,
        universalClosure(simplifyPropositional(
    ...
    def handleTacticBlock(block: ProofState => ProofState): Unit = {
      ...
      auxLemma(lemName, spec) {
        insert {
          ...
          ProofBuilder.c(LogicalAxiom(instantiate(spec, xs))).u(ExistsSkLeftRule(_, skC(xs), skDef)).u(ForallLeftBlock(_, desc, xs)).u(ForallRightBlock(_, spec, xs)).u(rules.CutRule(exProof, _, desc)).qed
        }
      }
  ...
  protected object indfn {
    inline def apply[U](constName: String, desc: Formula)(inline tacticsBlock: => Tactic[U]): Unit = new indfn(constName, desc).applyTactics(tacticsBlock)
  }
...
"""
  ...
  protected case class lemma(openFormula: Formula, attributes: String*)(implicit name: sourcecode.Name) extends LemmaHelper[LemmaHandle] {
  ...
  protected object lemma {
    inline def apply[U](openFormula: Formula, attributes: String*)(inline tacticsBlock: => Tactic[U])(implicit name: sourcecode.Name): LemmaHandle =
      new lemma(openFormula, attributes*).applyTactics(tacticsBlock)
  }
