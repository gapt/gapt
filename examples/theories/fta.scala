...
object fta extends Theory( natlists, listlength, natdivisible ) {
  ...
  val primelistdvdcnt = lemma( hof"lall prime d1 & lall prime d2 -> dvd (prod d1) (prod d2) <-> !k cnt(k,d1)<=cnt(k,d2)" ) {
    ...
    // case 1: x not in d2
    cut( "dvdxd2", hof"-dvd(x,prod(d2))" ); simp.h.on( "dvdxd2" ).w( "dvdnotelemprod", "elem" )
    cut( "dvdxd1d2", hof"-dvd(x*prod(d1_0),prod(d2))" ); include( "dvdtrans", "dvdmul" ); escrgt
    simp.h; exR( le"x:nat" ).forget; simp.h
    // case 2: x in d2
    cut( "pd2", hof"prod(d2) = prod(cons(x, del(x, d2)))" ); forget( "g_1" ); include( "prodperm" ); chain( "prodperm" ); simp.h( "elem" )
    cut( "g", hof"!(k:nat) (cnt k d1_0 <= cnt k (del x d2) <-> cnt k (cons x d1_0) <= cnt k d2)" ) right simp.h( "primene0", "lalldel" ); forget( "g_1" )
  ...
  val primedecexstep = lemma( hof"!m(m<n -> m!=0 -> ?d primedec d m) -> (n!=0 -> ?d primedec d n)" ) {
    impR; impR; cut( "pn", hof"-prime n" ); negR( "pn" )
    exR( le"cons (n:nat) nil" ).forget; simp.h( "primedec" )
    cut( "n1", hof"n!=s(0)" ); negR( "n1" )
    exR( le"nil: list nat" ).forget; simp.h( "primedec" )
  ...
  val primedecex = lemma( hof"n!=0 -> ?d primedec d n" ) {
    cut( "gen", hof"-!m (m<=n -> m!=0 -> ?d primedec d m)" ); decompose; chain( "gen" ); simp.on( "g_1" ); prop; forget( "g" ); negL
