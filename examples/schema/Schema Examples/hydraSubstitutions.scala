...
/**
  ...
  */
  ...
  /* 
    Implemented using the rule definitions from the original paper. (Using the succesor instead of the predecessor)
   */


object hydraSubstitutions extends TacticsProof {
  ...
  def simulateHydra(h1: Int, h2: Int, steps: Int): (List[(Int, Int)], List[Map[String, String]]) = {
    ...
    def nextStateWithSubstitution(
      ...
      if (n >= 1 && m >= 2) {
        // Rule B2: (n, m) -> (n-1, m-2)
        val next = (n - 1, m - 2)
        val subs = Map(s"X($xIndex)" -> (n - 1).toString, s"Y($yIndex)" -> (m - 2).toString)
        Some((next, "B2", subs, xIndex + 1, yIndex + 1))

      } else if (n == 0 && m >= 2) {
        // Rule C3: (0, m) -> (m-1, m-2)
        val next = (m - 1, m - 2)
        val subs = Map(s"Y($yIndex)" -> (m - 2).toString)
        Some((next, "C3", subs, xIndex, yIndex + 1))

      } else if (m == 0 && n >= 2) {
        // Rule D4: (n, 0) -> (n-1, n-2)
        val next = (n - 1, n - 2)
        val subs = Map(s"X($xIndex)" -> (n - 2).toString)
 ...
 /** Example call:
    ...
    hydraSubstitutions.exportSubstitutionHistoriesTxtRange(
        h1Range = 0 to 10,
        h2Range = 0 to 10,
        steps = 50,
        outputPath = "/Users/admin/Documents/hydra/hydra_substitutions1.txt"
      )
  * */
