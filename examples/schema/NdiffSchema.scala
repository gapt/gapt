...
object NdiffSchema extends TacticsProof {
  ...
  ctx += "nEqual" -> hos"E(f(AP(M,K)), f(AP(N,K))), E(f(B),N),E(f(AP(N,K)),M), LEQ(AP(N,K),B) :-"
  ctx += "leq_g" -> hos"LEQ(AP(s(X),Y),q):- LE(AP(X,Y),q)"
  ctx += "leq_refl" -> hos" :- LEQ(P,P)"
  ctx += "int_def" -> hos"LEQ(AP(M, R), AP(K, R)), E(f(AP(K, R)), M) :- E(f(AP(K, R)), K)"
  val esOmega = Sequent(
  ...
  val omegaBc = Lemma( esOmegaBc ) {
    ...
    exR( "Suc_0_0", le"AP(s(0), K)" )
    negR
    andL
    allL( "Ant_0_0", le"AP(s(0), K)" )
    andL
    andL
    foTheory
