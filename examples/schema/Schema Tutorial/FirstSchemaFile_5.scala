...
//however, even though context is duplicated it is ignored by the trivial rule, but that leaves another problem, the second open goal is not trivial. How do we close the proof. See FirstSchemaFile_6.scala
}
