...
object ringinv extends TacticsProof {
  ...
  val multleftCompatibility = Lemma(
  ...
  val setMultClosed = Lemma(
    Sequent(
      Seq("r-is-ring" -> hof"RINGAX", "multclosed" -> hof"∀(x:i)∀(y:i) ((R x ∧ R y) → R(x * y))"),
  ...
  val multleftDistributive = Lemma(
    Sequent(
  ...
  val multsetAssoc = Lemma(
    Sequent(
      Seq("extens" -> hof"∀A∀B ((A = B) <-> ∀(z:i) (A z <-> B z))", "multassoc" -> hof"∀(x:i)∀(y:i)∀(z:i) ((R x ∧ R y ∧ R z) → ((x * y) * z) = (x * (y * z)))"),
  ...
  val setMult1_Equal = Lemma(
    Sequent(
      Seq(("extens" -> hof"∀A∀B ((A = B) <-> ∀(z:i) (A z <-> B z))"), ("multleftneutral" -> hof"∀(x:i) (R x → (x = 1 * x))")),
