...
object def_prop_29 extends TipProblemDefinition {
  ...
  override def functions = List(
    ...
    TipFun(
      hoc"'ins1' :Nat>list>list",
      List(
        hof"(ins1(x:Nat, nil:list): list) = cons(x, nil)",
        hof"¬equal(x:Nat, z:Nat) ⊃   (ins1(x, cons(z, xs:list): list): list) = cons(z, ins1(x, xs))",
        hof"equal(x:Nat, z:Nat) ⊃ (ins1(x, cons(z, xs:list): list): list) = cons(z, xs)" ) ),
    TipFun(
      hoc"'elem' :Nat>list>o",
      List(
  ...
  override def goal = hof"∀x ∀xs elem(x:Nat, ins1(x, xs:list): list)"
}
