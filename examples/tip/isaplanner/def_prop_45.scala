...
object def_prop_45 extends TipProblemDefinition {
  ...
  override def goal = hof"∀x   ∀y   ∀xs   ∀ys   (#c(zip: list2>list2>list)(cons2(x:sk, xs:list2): list2, cons2(y, ys)):       list) =     cons(Pair2(x, y): Pair, #c(zip: list2>list2>list)(xs, ys): list)"
}
