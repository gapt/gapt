...
object prop_44 extends TacticsProof {
  ...
  val sequent =
    hols"""
      ...
      def_zip_2: ∀z   ∀x2   ∀x3   ∀x4   (#c(zip: list2>list2>list)(cons2(z:sk, x2:list2): list2, cons2(x3, x4))) =
           cons(Pair2(z, x3): Pair, #c(zip: list2>list2>list)(x2, x4): list),
      def_zipConcat_0: ∀x   ∀y   (#c(zipConcat: sk>list2>list2>list)(x:sk, y:list2, nil2:list2): list) = nil,
      def_zipConcat_1: ∀x   ∀y   ∀y2   ∀ys   (#c(zipConcat: sk>list2>list2>list)(x:sk, y:list2,
               cons2(y2:sk, ys:list2): list2):       list) =
                    cons(Pair2(x, y2): Pair, #c(zip: list2>list2>list)(y, ys): list),
      constr_inj_0: ∀y0 ∀y1 ¬(nil2:list2) = cons2(y0:sk, y1:list2),
      constr_inj_1: ∀y0 ∀y1 ¬(nil:list) = cons(y0:Pair, y1:list)
      :-
      goal: ∀x   ∀xs   ∀ys   (#c(zip: list2>list2>list)(cons2(x:sk, xs:list2): list2, ys:list2): list) =
           #c(zipConcat: sk>list2>list2>list)(x, xs, ys)
  """
