...
object def_prop_48 extends TipProblemDefinition {
  ...
  override def functions = List(
    TipFun(
      ...
      List(
        hof"null(nil:list): o",
        hof"¬null(cons(y:Nat, z:list): list)" ) ),
    TipFun(
      hoc"'last' :list>Nat",
      List(
    ...
    TipFun(
      ...
      List(
        ...
        hof"(butlast(cons(y:Nat, nil:list): list): list) = nil",
        hof"(butlast(cons(y:Nat, cons(x2:Nat, x3:list): list)): list) =   cons(y, butlast(cons(x2, x3)))" ) ),
    TipFun(
  ...
  override def goal = hof"∀xs   (¬null(xs:list) ⊃     (append(butlast(xs): list, cons(last(xs): Nat, nil:list): list): list) = xs)"
}
