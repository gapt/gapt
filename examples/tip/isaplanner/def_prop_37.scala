...
object def_prop_37 extends TipProblemDefinition {
  ...
  override def functions = List(
    ...
    TipFun(
      hoc"'delete' :Nat>list>list",
      List(
        hof"(delete(x:Nat, nil:list): list) = nil",
        hof"¬equal(x:Nat, z:Nat) ⊃   (delete(x, cons(z, xs:list): list): list) = cons(z, delete(x, xs))",
        hof"equal(x:Nat, z:Nat) ⊃ (delete(x, cons(z, xs:list): list): list) = delete(x, xs)" ) ) )

  override def goal = hof"∀x ∀xs ¬elem(x:Nat, delete(x, xs:list): list)"
}
