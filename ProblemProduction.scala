...
object ProblemProduction {
  ...
  def getProd( name: String, seqName: String ) =
    ...
    val indProofs = ( Map()
      ...
      // + getIsaplanner( "11", "sequent" ) // first problematic case, Requires changing vars to constants in the term construction
    ...
    val asTptpProbs = produceProblems.map(p =>{
      ...
      try{
        ...
        val ( folProblem, back ) = reduction forward (p._2)
