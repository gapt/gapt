...
abstract class RegressionTestCase(val name: String) extends Serializable {
  ...
  def run(): TestRun = {
    ...
    try testRun.runStep(None, timeout)(test(using testRun))
    ...
    val output = withTempFile { inputFile =>
      withTempFile { outputFile =>
        ...
        (Seq(javaBinary.toString) ++
          extraJvmArgs ++
          Seq("-cp", System.getProperty("java.class.path"), getClass.getCanonicalName.dropRight(1), inputFile.toString, outputFile.toString)) !
