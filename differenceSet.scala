...
object differenceSet {

  // List of different pairs. It could contain also non-unifiable pairs (for instance: (c,fc) for a constant c and a function f).
  private var differenceList = ListBuffer[( LambdaExpression, LambdaExpression )]()

  // Initializes the computation of the list of different pairs.
  def apply( termL: LambdaExpression, termR: LambdaExpression ): ListBuffer[( LambdaExpression, LambdaExpression )] = {
    var differenceSet = ListBuffer[( LambdaExpression, LambdaExpression )]()

    differenceSet += ( ( termL, termR ) )
  ...
  // Main recursive call.
  private def walk( differenceSet: scala.collection.mutable.ListBuffer[( LambdaExpression, LambdaExpression )] ): scala.collection.mutable.ListBuffer[( LambdaExpression, LambdaExpression )] = {
    ...
    val ( differenceSetL: LambdaExpression, differenceSetR: LambdaExpression ) = differenceSet.head
    ...
    if ( fL == fR ) {
      ...
      for ( i <- 1 to argsL.length ) {
        differenceSetIterator += ( ( argsL.apply( i - 1 ), argsR.apply( i - 1 ) ) )
      }
